...
int main(void)
{
	...
	if (have_assignable) {
		...
		if (ok) {
			...
			memset(&r, 0, sizeof(r));
			if (kx_client_request(&client, "POST", "/v1/containers/devlive/devices", body, &r) !=
			        0 ||
			    r.status != 200) {
		...
		if (ok) {
			...
			memset(&r, 0, sizeof(r));
			if (kx_client_request(&client, "DELETE", path, NULL, &r) != 0 || r.status != 200) {
		...
		if (ok) {
			...
			if (ok) {
				...
				snprintf(path, sizeof(path), "/v1/containers/devlivecreate/devices/%s",
				         first_assignable_id);
				memset(&r, 0, sizeof(r));
				if (kx_client_request(&client, "DELETE", path, NULL, &r) != 0 ||
				    r.status != 409) {
					fprintf(stderr,
			...
			memset(&r, 0, sizeof(r));
			kx_client_request(&client, "DELETE", "/v1/containers/devlivecreate", NULL, &r);
			kx_response_free(&r);
		}

		memset(&r, 0, sizeof(r));
		kx_client_request(&client, "DELETE", "/v1/containers/devlive", NULL, &r);
		kx_response_free(&r);
	} else {
	...
	memset(&r, 0, sizeof(r));
	if (kx_client_request(&client, "POST", "/v1/containers/nosuchcontainer/devices",
	...
	memset(&r, 0, sizeof(r));
	if (kx_client_request(&client, "DELETE", "/v1/containers/nosuchcontainer/devices/usb:x", NULL,
	                       &r) != 0 ||
	    r.status != 404) {
