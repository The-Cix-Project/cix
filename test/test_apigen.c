...
int main(void)
{
	...
	else if (atoi(out) != 292)
		fail("apigen found %d operations in the real spec, expected 292 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 292)
			fail("CLI header has %d path defines, expected one per operation (292)", defines);
		if (methods != 292)
			fail("CLI header has %d method defines, expected one per operation (292)", methods);
		if (braces != 0)
