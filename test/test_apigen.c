...
int main(void)
{
	...
	else if (atoi(out) != 268)
		fail("apigen found %d operations in the real spec, expected 268 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 268)
			fail("CLI header has %d path defines, expected one per operation (268)", defines);
		if (methods != 268)
			fail("CLI header has %d method defines, expected 268", methods);
		if (braces != 0)
