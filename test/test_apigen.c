...
int main(void)
{
	...
	else if (atoi(out) != 296)
		fail("apigen found %d operations in the real spec, expected 296 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 296)
			fail("CLI header has %d path defines, expected one per operation (296)", defines);
		if (methods != 296)
			fail("CLI header has %d method defines, expected one per operation (296)", methods);
		if (braces != 0)
