...
int main(void)
{
	...
	else if (atoi(out) != 289)
		fail("apigen found %d operations in the real spec, expected 289 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 289)
			fail("CLI header has %d path defines, expected one per operation (289)", defines);
		if (methods != 289)
			fail("CLI header has %d method defines, expected one per operation (289)", methods);
		if (braces != 0)
