...
int main(void)
{
	...
	else if (atoi(out) != 293)
		fail("apigen found %d operations in the real spec, expected 293 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 293)
			fail("CLI header has %d path defines, expected one per operation (293)", defines);
		if (methods != 293)
			fail("CLI header has %d method defines, expected one per operation (293)", methods);
		if (braces != 0)
