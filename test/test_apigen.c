...
int main(void)
{
	...
	else if (atoi(out) != 307)
		fail("apigen found %d operations in the real spec, expected 307 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 307)
			fail("CLI header has %d path defines, expected one per operation (307)", defines);
		if (methods != 307)
			fail("CLI header has %d method defines, expected one per operation (307)", methods);
		if (braces != 0)
