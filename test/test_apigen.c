...
int main(void)
{
	...
	else if (atoi(out) != 267)
		fail("apigen found %d operations in the real spec, expected 267 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 267)
			fail("CLI header has %d path defines, expected one per operation (267)", defines);
		if (methods != 267)
			fail("CLI header has %d method defines, expected 267", methods);
		if (braces != 0)
