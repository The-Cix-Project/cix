...
int main(void)
{
	...
	else if (atoi(out) != 291)
		fail("apigen found %d operations in the real spec, expected 291 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 291)
			fail("CLI header has %d path defines, expected one per operation (291)", defines);
		if (methods != 291)
			fail("CLI header has %d method defines, expected one per operation (291)", methods);
		if (braces != 0)
