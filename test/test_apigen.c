...
int main(void)
{
	...
	else if (atoi(out) != 271)
		fail("apigen found %d operations in the real spec, expected 271 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 271)
			fail("CLI header has %d path defines, expected one per operation (271)", defines);
		if (methods != 271)
			fail("CLI header has %d method defines, expected 271", methods);
		if (braces != 0)
