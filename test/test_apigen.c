...
int main(void)
{
	...
	else if (atoi(out) != 274)
		fail("apigen found %d operations in the real spec, expected 274 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 274)
			fail("CLI header has %d path defines, expected one per operation (274)", defines);
		if (methods != 274)
			fail("CLI header has %d method defines, expected 274", methods);
		if (braces != 0)
