...
int main(void)
{
	...
	else if (atoi(out) != 290)
		fail("apigen found %d operations in the real spec, expected 290 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 290)
			fail("CLI header has %d path defines, expected one per operation (290)", defines);
		if (methods != 290)
			fail("CLI header has %d method defines, expected one per operation (290)", methods);
		if (braces != 0)
