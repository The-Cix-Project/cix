...
int main(void)
{
	...
	else if (atoi(out) != 269)
		fail("apigen found %d operations in the real spec, expected 269 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 269)
			fail("CLI header has %d path defines, expected one per operation (269)", defines);
		if (methods != 269)
			fail("CLI header has %d method defines, expected 269", methods);
		if (braces != 0)
