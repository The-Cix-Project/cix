...
int main(void)
{
	...
	else if (atoi(out) != 281)
		fail("apigen found %d operations in the real spec, expected 281 -- if the spec "
	 ...
	 */
	{
		...
		if (defines != 281)
			fail("CLI header has %d path defines, expected one per operation (281)", defines);
		if (methods != 281)
			fail("CLI header has %d method defines, expected one per operation (281)", methods);
		if (braces != 0)
