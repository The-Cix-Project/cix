...
int main(void)
{
	 ...
	 */
	memset(&r, 0, sizeof(r));
	check(kx_client_request(&client, "POST", "/v1/volumes", "{\"name\":\"execvol\"}", &r) == 0 &&
	          r.status == 201,
	...
	memset(&r, 0, sizeof(r));
	check(kx_client_request(&client, "POST", "/v1/containers/cexec/volumes",
	...
	memset(&r, 0, sizeof(r));
	check(kx_client_request(&client, "POST", "/v1/containers/cexec/exec",
...

	{
		...
		for (tries = 0; tries < 100; tries++) {
			usleep(100000);
			memset(&r, 0, sizeof(r));
			if (kx_client_request(&client, "GET", "/v1/containers/cexec/exec", NULL, &r) != 0) {
				kx_response_free(&r);
				continue;
			}
	 ...
	 */
	memset(&r, 0, sizeof(r));
	kx_client_request(&client, "DELETE", "/v1/containers/cexec", NULL, &r);
	kx_response_free(&r);
	...
	memset(&r, 0, sizeof(r));
	kx_client_request(&client, "DELETE", "/v1/containers/cexecread", NULL, &r);
	kx_response_free(&r);
	wait_gone(&client, "cexecread");
	memset(&r, 0, sizeof(r));
	kx_client_request(&client, "DELETE", "/v1/volumes/execvol", NULL, &r);
	kx_response_free(&r);
