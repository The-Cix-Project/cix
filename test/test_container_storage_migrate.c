...
int main(void)
{
	...
	memset(&r, 0, sizeof(r));
	if (kx_client_request(&client, "GET", "/v1/containers/nosuchcontainer/migrate-storage", NULL, &r) !=
	            0 ||
	    r.status != 404) {
	...
	memset(&r, 0, sizeof(r));
	if (kx_client_request(&client, "POST", "/v1/containers/nosuchcontainer/migrate-storage",
	                       "{\"disk\":null}", &r) != 0 ||
	    r.status != 404) {
	...
	memset(&r, 0, sizeof(r));
	kx_client_request(&client, "DELETE", "/v1/containers/noreplay", NULL, &r);
	kx_response_free(&r);
	...
	memset(&r, 0, sizeof(r));
	if (kx_client_request(&client, "GET", "/v1/containers/migc/migrate-storage", NULL, &r) != 0 ||
	    r.status != 200) {
	...
	memset(&r, 0, sizeof(r));
	if (kx_client_request(&client, "POST", "/v1/containers/migc/migrate-storage", "{}", &r) != 0 ||
	    r.status != 400) {
	...
	memset(&r, 0, sizeof(r));
	if (kx_client_request(&client, "POST", "/v1/containers/migc/migrate-storage", "{\"disk\":null}",
	                       &r) != 0 ||
	    r.status != 409) {
		fprintf(stderr,
	...
	memset(&r, 0, sizeof(r));
	if (kx_client_request(&client, "POST", "/v1/containers/migc/migrate-storage",
	...
	if (non_os_disk[0] == '\0') {
		...
		memset(&r, 0, sizeof(r));
		if (kx_client_request(&client, "POST", "/v1/containers/migc/migrate-storage", body, &r) != 0 ||
		    r.status != 400) {
	...
	/* Cleanup. */
	memset(&r, 0, sizeof(r));
	kx_client_request(&client, "DELETE", "/v1/containers/migc", NULL, &r);
	kx_response_free(&r);
