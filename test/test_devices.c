 ...
 *    a struct kx_bpf_insn byte-packing bug from a device-logic bug,
...
/* Isolates a struct kx_bpf_insn byte-packing bug from a device-logic
...
{
	struct kx_bpf_insn prog[2];
	struct kx_bpf_prog_load_attr attr;
	int fd;
	...
	attr.prog_type = KX_BPF_PROG_TYPE_CGROUP_DEVICE;
	attr.insn_cnt = 2;
	attr.insns = (uint64_t)(uintptr_t)prog;
	attr.license = (uint64_t)(uintptr_t)"GPL";
	attr.expected_attach_type = KX_BPF_CGROUP_DEVICE;

	fd = (int)sys_bpf(KX_BPF_PROG_LOAD, &attr, sizeof(attr));
	if (fd < 0) {
