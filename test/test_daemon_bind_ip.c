 ...
 * field -- verified via a full live round-trip against a real cixd:
 ...
 * bridge), confirmed both via reconnecting cix_client instances at each
...
{
	pid_t daemon_pid;
	struct cix_client client_initial, client_net_addr, client_bind_ip, client_fallback;
	int ok = 1;
	struct cix_response r;
	...
	cix_client_init(&client_initial, "127.0.0.1", TEST_PORT);
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client_initial, "POST", "/v1/networks",
	 ...
	 * address. */
	memset(&r, 0, sizeof(r));
	if (ok && (cix_client_request(&client_initial, "PUT", "/v1/system/daemon-config",
	...
	cix_client_init(&client_net_addr, NET_ADDR, TEST_PORT);
	...
	memset(&r, 0, sizeof(r));
	if (ok && (cix_client_request(&client_net_addr, "PUT", "/v1/system/daemon-config",
	                              "{\"bind_ip\":\"" BIND_IP "\"}", &r) != 0 ||
	           r.status != 200)) {
	...
	cix_client_init(&client_bind_ip, BIND_IP, TEST_PORT);
	...
	memset(&r, 0, sizeof(r));
	if (ok && (cix_client_request(&client_bind_ip, "GET", "/v1/system/daemon-config", NULL, &r) != 0 ||
	           r.status != 200)) {
	 ...
	 * bind_ip's existence. */
	memset(&r, 0, sizeof(r));
	if (ok && (cix_client_request(&client_bind_ip, "GET",
	 ...
	 * unreferenced. */
	memset(&r, 0, sizeof(r));
	if (ok && (cix_client_request(&client_bind_ip, "PUT", "/v1/system/daemon-config",
	                              "{\"bind_ip\":null}", &r) != 0 ||
	           r.status != 200)) {
	...
	cix_client_init(&client_fallback, NET_ADDR, TEST_PORT);
	...
	memset(&r, 0, sizeof(r));
	if (ok && (cix_client_request(&client_fallback, "GET", "/v1/system/daemon-config", NULL, &r) != 0 ||
	           r.status != 200)) {
