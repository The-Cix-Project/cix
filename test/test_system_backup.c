...
static time_t wait_for_present(const struct cix_client *c, const char *name, int max_attempts)
{
	int attempts;
...
static time_t wait_for_absent(const struct cix_client *c, const char *name, int max_attempts)
{
	int attempts;
...
int main(void)
{
	 ...
	 * and content asserted below, is what would have caught that. */
	{
		...
		memset(&r, 0, sizeof(r));
		if (cix_client_request(&client, "POST", "/v1/pkg/recipes", rw.buf, &r) != 0 ||
		    r.status != 204) {
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "POST", "/v1/volumes", "{\"name\":\"backupvol\"}", &r) != 0 ||
	    r.status != 201) {
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "GET", "/v1/system/backup", NULL, &r) != 0 || r.status != 200) {
		...
		if (recipes == NULL || recipes->type != JSON_OBJECT) {
			...
			const struct json_value *entry = json_object_get(recipes, "backuptestpkg/1.0-1/build.cbs");
			...
			if (content == NULL || strstr(content, "package \"backuptestpkg\"") == NULL) {
				...
				fprintf(stderr,
				        "FAIL: backup pkg_recipes missing \"backuptestpkg/1.0-1/build.cbs\" "
				        "key with real content (ADR-0120 regression: version-keyed layout "
				        "not walked)\n");
				/* What IS in there. The key is the recipe store's own
				 * <name>/<version>/<filename> shape -- and a CPDL recipe changes
				 * both halves at once: the release joins the version, and build.sh
				 * becomes build.cbs. An assertion written for one format then fails
				 * against the other with no way to tell a wrong key from an absent
				 * recipe (cix#516). */
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "DELETE", "/v1/networks/backupnet", NULL, &r) != 0 ||
	    r.status != 204) {
	...
	cix_response_free(&r);
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "DELETE", "/v1/dns/records/backup.internal", NULL, &r) != 0 ||
	    r.status != 204) {
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "POST", "/v1/system/restore",
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "POST", "/v1/system/restore", "{}", &r) != 0 ||
	    r.status != 400) {
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "DELETE", "/v1/containers/keeper", NULL, &r) != 0 ||
	    r.status != 204) {
	...
	if (saved_bundle != NULL) {
		...
		memset(&r, 0, sizeof(r));
		if (cix_client_request(&client, "POST", "/v1/system/restore", restore_body.buf, &r) != 0 ||
	 ...
	 * find again. */
	{
		...
		memset(&r, 0, sizeof(r));
		if (cix_client_request(&client, "POST", "/v1/system/restore", recipe_restore.buf, &r) !=
		        0 ||
		    r.status != 200) {
