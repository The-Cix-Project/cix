...
#define VETH_A "cix-nif-a"
#define VETH_B "cix-nif-b"
...
int main(void)
{
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "POST", "/v1/networks/nifnet/interfaces",
	                       "{\"ifname\":\"cix-nif-nonexistent\"}", &r) != 0 ||
	    r.status != 400) {
	...
	memset(&r, 0, sizeof(r));
	{
		...
		if (cix_client_request(&client, "POST", "/v1/networks/nifnet/interfaces", body, &r) != 0 ||
		    r.status != 400) {
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "POST", "/v1/networks/nosuchnet/interfaces",
	                       "{\"ifname\":\"cix-nif-a\"}", &r) != 0 ||
	    r.status != 404) {
	...
	memset(&r, 0, sizeof(r));
	if (cix_client_request(&client, "GET", "/v1/networks/nifnet", NULL, &r) != 0 || r.status != 200) {
