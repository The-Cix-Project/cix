...
int main(void)
{
	...
	memset(&r, 0, sizeof(r));
	if (kx_client_request(&client, "GET", "/v1/system/sysctl/net.ipv4.ip_forward", NULL, &r) != 0 ||
	    r.status != 200) {
	...
	memset(&r, 0, sizeof(r));
	if (ok && (kx_client_request(&client, "GET", "/v1/system/sysctl/net.ipv4.ip_local_port_range",
	                              NULL, &r) != 0 ||
	           r.status != 200)) {
	...
	if (ok) {
		...
		if (kx_client_request(&client, "PUT", "/v1/system/sysctl/net.ipv4.ip_forward", body, &r) !=
		        0 ||
		    r.status != 200) {
	 ...
	 * one persisted key. */
	memset(&r, 0, sizeof(r));
	if (ok && (kx_client_request(&client, "GET", "/v1/system/sysctl", NULL, &r) != 0 ||
	           r.status != 200)) {
	...
	if (ok && port_range_value[0] != '\0') {
		...
		if (sscanf(port_range_value, "%31s %31s", lo, hi) == 2) {
			...
			if (kx_client_request(&client, "PUT", "/v1/system/sysctl/net.ipv4.ip_local_port_range",
			                       body, &r) != 0 ||
			    r.status != 200) {
	...
	memset(&r, 0, sizeof(r));
	if (ok && (kx_client_request(&client, "DELETE", "/v1/system/sysctl/net.ipv4.ip_forward", NULL,
	                              &r) != 0 ||
	           r.status != 204)) {
	...
	memset(&r, 0, sizeof(r));
	if (ok && (kx_client_request(&client, "GET", "/v1/system/sysctl/net.ipv4.ip_forward", NULL, &r) !=
	               0 ||
	           r.status != 200)) {
	...
	memset(&r, 0, sizeof(r));
	if (ok && (kx_client_request(&client, "PUT", "/v1/system/sysctl/net.ipv4.ip_forward", "{}", &r) !=
	               0 ||
	           r.status != 400)) {
	...
	memset(&r, 0, sizeof(r));
	if (ok && (kx_client_request(&client, "GET", "/v1/system/sysctl/bad/key", NULL, &r) != 0 ||
	           r.status != 400)) {
