...
int main(void)
{
	...
	if (test_data_dir_create(g_data_dir, sizeof(g_data_dir)) != 0)
		return 1;
	snprintf(g_image_root, sizeof(g_image_root), "%s/rebuildable/images/lifecycletest/v1/rootfs", g_data_dir);
	{
		char image_dir[PATH_MAX];

		snprintf(image_dir, sizeof(image_dir), "%s/rebuildable/images/lifecycletest", g_data_dir);
	 ...
	 * "stopped", not just "gone." This is the actual bug reported. */
	{
		...
		if (cix_client_request(&client, "PATCH", "/v1/containers/lc1",
		                       "{\"services\":[{\"name\":\"main\",\"on_exit\":\"fail-container\",\"cmd\":[\"/bin/daemon_child\",\"45\",\"0\"]}],"
		                       "\"env\":{\"PATCHED\":\"yes\"}}",
		                       &r) != 0 ||
	 ...
	 * observe the definition while nothing is live for it. */
	{
		...
		if (cix_client_request(&client, "POST", "/v1/containers",
		                       ...
		                       "\"services\":[{\"name\":\"main\",\"on_exit\":\"fail-container\",\"cmd\":[\"/bin/no-such-binary-here\"]}],"
	 ...
	 * deliberately distinguishable (opted out vs. captured-but-empty). */
	{
		...
		if (cix_client_request(&client, "POST", "/v1/containers",
		                       ...
		                       "\"services\":[{\"name\":\"main\",\"on_exit\":\"fail-container\",\"cmd\":[\"/bin/output_child\"]}],"
		                       "\"capture_output\":true}",
		                       &r) != 0 ||
		    r.status != 201) {
