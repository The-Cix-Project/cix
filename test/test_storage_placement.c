...
int main(void)
{
	...
	if (non_os_disk[0] == '\0') {
		...
		memset(&r, 0, sizeof(r));
		if (cix_client_request(&client, "GET", "/v1/system/rebuildable-storage", NULL, &r) != 0 ||
		    r.status != 200) {
		...
		snprintf(body, sizeof(body), "{\"disk\":\"%s\"}", non_os_disk);
		memset(&r, 0, sizeof(r));
		if (cix_client_request(&client, "POST", "/v1/system/rebuildable-storage/migrate", body, &r) !=
		            0 ||
		    r.status != 400) {
			fprintf(stderr,
		...
		memset(&r, 0, sizeof(r));
		if (cix_client_request(&client, "GET", "/v1/system/rebuildable-storage/migrate", NULL, &r) !=
		            0 ||
		    r.status != 200) {
