...
- **CLI:** `kanxeoctl --host=<ip> health`, `... ps`, `... run --name=... --image=... --network=... -- CMD`, `... network create --name=... --subnet=... --prefix=...` (see `docs/api/README.md` for the full walkthrough). Running `kanxeoctl --host=<ip>` with no command, from a real terminal (on the console or over SSH), opens an interactive shell instead — type commands one per line (`ps`, `run --name=... -- CMD`, etc.) without re-typing `kanxeoctl --host=...` each time; `exit`/`quit`/Ctrl-D end it. `--host=`/`--port=`/`--json` are set once at startup and apply to the whole session.
- **Video console:** the installed system now shows boot and login output on the machine's own video output (Proxmox's console viewer, a real monitor), not just the serial port — driven by the EFI framebuffer UEFI firmware already sets up, so it works without any GPU-specific driver bound to anything.
