...
- The Bash tool's default shell runs as an unprivileged, sandboxed user (`id` shows a non-root uid, `kernel.unprivileged_bpf_disabled=2`) — every `kanxeod`/test binary that actually exercises `clone3`/`mount`/`pivot_root`/`bpf(BPF_PROG_LOAD, ...)` (i.e. anything that creates a real container, not just compiles code) needs both `sudo` (passwordless, confirmed working) and the Bash tool's `dangerouslyDisableSandbox: true`; without both, container creation fails opaquely with a generic "failed to create container" 500 and no logged errno, which looks like a code bug but isn't (confirmed by tracing to `bpf(BPF_PROG_LOAD, ...) = -1 EPERM` via `strace`, then confirming a plain device-less container creation *also* silently fails the same way under the sandboxed shell). Building and running plain host-side tools (no container creation) works fine without either.
...
- `kanxeod` accepts `--data-dir=PATH` (default unchanged: `/var/lib/kanxeo`) specifically so this project's own test suite can never collide with a real deployment's live state again — every one of the 16 daemon-linked tests (`test_daemon.c`, `test_cli.c`, `test_web.c`, `test_daemon_net.c`, `test_networks.c`, `test_network_interfaces.c`, `test_images.c`, `test_container_restart.c`, `test_container_files.c`, `test_dns.c`, `test_pki.c`, `test_pkg.c`, `test_daemon_devices.c`, `test_system_update.c`, `test_system_backup.c`, `test_console_exec.c`) now spawns its own `kanxeod` subprocess against a fresh `mkdtemp("/tmp/kanxeo_test_data_XXXXXX")` directory (`test_data_dir_create()`/`test_data_dir_cleanup()`, `test/test_image_fixture.c`) instead of the real default path. This exists because running this suite against a host that also has a real `kanxeod` on default paths used to silently wipe that daemon's live container/network/DNS/PKI state and image content — confirmed the hard way, twice, in the same session. Any new daemon-linked test must follow this same pattern, not hand-roll a bare `dargv[]` pointed at the default path.
