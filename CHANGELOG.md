...
### Part 165 (in progress): `cgroup_create()` gains real per-failure diagnostics -- a genuine ADR-0165 production regression found, not yet root-caused

Every ordinary `pkg install` against 192.168.15.95 failed 100% of the time once ADR-0165's real `memory_max`/`cpu_max` limits went live (`could not start the build container`) -- disabling the limits immediately restored success, a real, confirmed regression. Local reproduction was inconclusive: this dev sandbox's systemd-managed cgroup hierarchy pre-delegates controllers the same way every earlier test environment already did, masking whatever the real box's genuine PID-1/no-systemd hierarchy is hitting. `cgroup_create()` had zero diagnostics on any of its five failure points -- fixed with a distinct `perror()` per path, reaching the log store the same way `cgroup_enable_controllers()`'s own diagnostics already do. **Limits deliberately left disabled on 192.168.15.95 pending this diagnostic build's deployment** -- re-enabling blind again would just repeat tonight's outage risk.
