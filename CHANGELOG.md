...
New `kanxeoctl pkg-build-config show|set --max-concurrent-jobs=N` (a top-level command, matching the REST path being under `/v1/system/`) and a new "Package Builds" web dashboard page under the existing Host nav group, both mirroring `rolling-config`'s own established shape.
...
- `kanxeoctl container network attach|detach`.
...
### Part 145 (done): task #865 closed for real -- the first real self-hosted kanxeod deploy to 192.168.15.95, and the real bugs it took to get there
...
#### Fixed
- **ADR-0154**: `mkbootroot`'s server-side `mksquashfs` invocation now sets `LD_LIBRARY_PATH` for a `kanxeo-hosttools`-sourced binary -- without it, the dynamic linker silently linked against whatever `liblzma.so.5` the bare host already had instead of the one actually built for it.
...
- `recipes/package/kanxeo/v1.54.0`: re-pinned (task #852) carrying the LD_LIBRARY_PATH fix -- the first tag that could actually deploy.

#### Verified
- Full self-hosted build+deploy cycle against the real box, start to finish: built `kanxeo-builder` (tcc/make/libc-dev/bash/coreutils/openssl) and `kanxeo-hosttools` (coreutils/gzip/openssl/zlib/curl/tar/bzip2/xz/squashfs-tools) images live, ran `pkg hostbuild kanxeo --deploy`, confirmed a real, `hsqs`-valid squashfs staged to the inactive slot, rebooted, confirmed the box came back on the new slot with all 7 containers auto-started.
...
### Part 144 (done): root-cause investigation of a real kanxeod hang on 192.168.15.95, swap enabled as mitigation (task #875)

Real, live incident: after a burst of legitimate work (several image-recipe applies, multiple `pkg install` calls) `kanxeod` on 192.168.15.95 stopped responding to HTTP entirely -- TCP `accept()` still succeeded instantly, but no request ever completed, for an extended period. No SSH/console access existed to investigate live; the box was eventually recovered by an out-of-band reboot (a real, hard-to-avoid loss of the in-memory forensic state -- the previous boot's kernel ring buffer and any further live diagnosis went with it).
...
- `pkg/recipes/kanxeo/v1.4.0/build.sh` re-pinned to `v1.49.0` (task #852) and `glauth/2.4.0/build.sh`'s dead `pkg_source` URL re-served, both real, separate gaps found and fixed while standing this infrastructure back up -- see the dedicated commit for full detail (`pkg_sha256`/access token in the `kanxeo` recipe are left as explicit placeholders, never a committed secret, matching that recipe's own established convention -- publishing it live still needs a real token this session has no credential for).
...
- `image/src/kanxeo-install.c`: prints a clear final banner ("Installation complete. Remove the installation media, then press Enter to reboot.") after `dual_console_wait_for_key()`, `sync()`s, and calls `reboot(RB_AUTOBOOT)` -- instead of returning from `main()` into an unprompted panic. A short note ahead of the `mokutil --import` call explains that its keyring warnings are expected and harmless in this minimal environment.

#### Verified
- `test/test_dual_console`: 5 consecutive clean runs, no flakes. Full clean rebuild (`-Wall -Werror`, zero warnings). `test/test_installer`'s full QEMU install-and-boot verification re-run against the changed `kanxeo-install` -- still PASS (the test's own `success_marker` match fires, and force-kills QEMU, before the new wait-for-key/reboot code path is reached, so this change is real but not exercised by that automated harness; verified by code review and the standalone `dual_console_wait_for_key()` test instead).
