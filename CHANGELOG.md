...
- **#29 Phase 1 done** (ADR-0179): the subordinate-ID range allocator (`daemon/src/subid.c`) -- a persisted `identity-key -> host-UID/GID range` table, floor 100000 / width 65536 (the Docker/Podman/systemd conventions), first-come per key, with active both-direction collision checking against real LDAP uidnumbers (the allocator steps past a colliding range; `ldap_user_create`/`update` reject a uidnumber inside a committed range). Wired into boot init and validated by a new `test/test_subid.c` (idempotence, non-overlap, floor, persistence-across-reload). **Phase 2 (the actual CLONE_NEWUSER + id-mapped-mount flip) is deliberately gated on the incoming bare-metal box** -- the ADR itself requires empirical verification of `mount_setattr(MOUNT_ATTR_IDMAP)` over this overlayfs+ext4 stack before flipping a security default, which the privileged-nested-LXC dev sandbox can't provide. `container_spec` carries the `userns_*` fields; nothing enables them yet.
