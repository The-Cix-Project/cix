 ...
 * by the user: kernel dmesg, cixd's own diagnostics, and a per-
 ...
 * "source": "kernel"|"cixd"|"audit", "level": "...", "msg": "..."}.
 * "source" distinguishes the real Linux kernel ring buffer (read from
 * /dev/kmsg), cixd's own internal diagnostics (its existing
 ...
 * that's genuinely the noisiest source today; cixd/audit's own
 ...
 * /dev/kmsg couldn't be opened (non-fatal -- logged as a cixd-
...
/*
 * Transparent container-log capture (added alongside kernel/cixd/
 ...
 * "error" (both accepted -- cixd/audit's own convention is
