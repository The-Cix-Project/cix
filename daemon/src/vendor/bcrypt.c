...
/*
 * Vendored from OpenBSD's own lib/libc/crypt/bcrypt.c (real, audited
 * reference implementation of the algorithm David Mazieres designed --
 * see daemon/include/pwhash.h for why this project vendors a real
 * implementation rather than hand-rolling bcrypt itself). Trimmed to
 * only what daemon/src/pwhash.c's own thin wrapper needs
 * (bcrypt_newhash/bcrypt_checkpass) -- bcrypt_gensalt()/bcrypt()/
 * _bcrypt_autorounds() (the "classic interface" + OpenBSD's own libc-
 * internal cost self-calibration) dropped, along with their own
 * OpenBSD-only dependencies (<pwd.h>'s _PASSWORD_LEN, libc's internal
 * WRAP() symbol-interposition macro) -- this project always specifies
 * an explicit cost factor (PWHASH_BCRYPT_COST, pwhash.h), no runtime
 * self-calibration needed. arc4random_buf()/explicit_bzero() are both
 * real glibc >= 2.36/2.25 functions respectively (this project's own
 * libc-dev.recipe is 2.36) -- no compat shim needed for either.
 * timingsafe_bcmp() is OpenBSD-libc-only; a real constant-time
 * byte-compare is implemented locally below rather than pulled in from
 * anywhere else, the same one-function-only scope every other small
 * compat gap in this codebase gets.
 */
...
/* Real constant-time compare -- OpenBSD's own timingsafe_bcmp(), not
 * available on Linux/glibc, reimplemented directly (one function,
 * the same shape as OpenBSD's own: accumulate XOR differences across
 * the whole length regardless of where a mismatch first occurs, never
 * an early return that would leak comparison-length timing). */
static int
pwhash_timingsafe_bcmp(const void *b1, const void *b2, size_t n)
{
...
bcrypt_checkpass(const char *pass, const char *goodhash)
{
	...
	if (strlen(hash) != strlen(goodhash) ||
	    pwhash_timingsafe_bcmp(hash, goodhash, strlen(goodhash)) != 0) {
		errno = EACCES;
		return -1;
	}
