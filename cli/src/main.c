...
static void print_usage(FILE *out)
{
	fprintf(out,
	        ...
	        "  volume owner NAME --uid=N --gid=N [--recursive] / volume owner NAME --root\n"
	        "               -- who owns the volume's root (#102); a new volume is root's,\n"
	        "               so a workload that is not root cannot write to it until set\n"
	        ...
	        "  image create --name=NAME  -- an empty image with no packages, not even a C\n"
	        "               library: install one before running a container on it,\n"
	        "               with pkg install --name=glibc --image=NAME\n"
	        "  image ls\n"
	        ...
	        "  ntp server register --container=NAME  -- registers a running container as an\n"
