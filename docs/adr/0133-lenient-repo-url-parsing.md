...
User-reported: `pkg sync` failed with `state: failed`, `curl exit 6` (`CURLE_COULDNT_RESOLVE_HOST`) at first — that half was a real, separate `resolv_set()` bug (ADR-0132). Once DNS resolution was fixed, the *same* sync attempt failed differently: `curl exit 22` (an HTTP error response), against a `repo_url` of `https://git.home.arpa/itdlabs/thinc/src/branch/master/pkg/recipes`.

`parse_repo_url()` (`daemon/src/pkg.c`) documented its own expected input as `<scheme>://<host>/<owner>/<repo>[.git][/]` and split owner/repo at the **last** `/` in the path. Against the URL above, the path is `itdlabs/thinc/src/branch/master/pkg/recipes` — the last slash falls between `pkg` and `recipes`, so `owner` became the entire garbage string `itdlabs/thinc/src/branch/master/pkg` and `repo` became `recipes`. The resulting gitea API fetch URL was nonsense, and gitea correctly 404'd it — reported to the operator as an opaque `curl exit 22` with no hint that the *URL itself*, not the network or the target repo, was the problem.
