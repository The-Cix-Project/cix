...
thinC already has a proven, working pattern for exactly this shape of problem, used twice: **thinC owns the durable record, and renders into the consumer's own filesystem/format** on every change — not the consumer querying thinC or a directory protocol live.
...
- `sshd` on a registered target has zero LDAP awareness. This is a deliberate, stated trade-off, not an oversight: it means thinC's own sync cadence (immediate on every mutation) is the actual latency between "an LDAP user's key is added" and "that key works on the target" — no caching, no TTL, but also no live query-time dependency on glauth being reachable at login time.
