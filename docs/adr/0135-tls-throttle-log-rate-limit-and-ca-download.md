...
- `thincctl tls-throttle show`/`set --log-interval-seconds=N` and the web dashboard's TLS Throttle page both gained the new field, same partial-update convention as every other field on this resource.
