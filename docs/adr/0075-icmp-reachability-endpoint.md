...
**`kanxeoctl ping HOST`** polls to completion client-side (same shape as `--wait` on `pkg hostbuild`/`iso build`) and exits nonzero on an unreachable result — a real, scriptable check, not just a status printer.
