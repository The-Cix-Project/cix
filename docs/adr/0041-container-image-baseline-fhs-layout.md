...
- Scoped to container images only — the control-plane root itself needs none of this (`CONFIG_DEVTMPFS=y`/`CONFIG_DEVTMPFS_MOUNT=y` already auto-populates `/dev` there at boot, and `kanxeod`'s own state lives entirely under `BASE_DIR`, never `/run`) — `image/src/mkbootroot.c`/`kanxeo-install.c` are unaffected, the same "two genuinely separate artifacts" framing ADR-0023 already established.
