...
**LDAP backend** (Part 113): `ldapclient.c` verified standalone against a real local `glauth` process — bind, wrong-password rejection, nonexistent-user rejection, closed-port failure, single- and multi-term search — every case cross-checked against `ldapsearch`/`ldapwhoami`, not just self-consistency. **Live-verified against the real deployed `glauth` servers** (192.168.15.95's `ldap-1`/`ldap-2`): deployed via this project's own self-hosted rebuild path (`pkg hostbuild kanxeo --deploy`), rebooted, confirmed the box came back on the new build, and a real throwaway LDAP user bound successfully (`resultCode=0`) against both servers directly.
