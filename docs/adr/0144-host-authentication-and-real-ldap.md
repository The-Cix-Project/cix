...
Investigated directly while following up on the container-SSH work (ADR-0143's own follow-on question, "can we SSH into jumpbox with a key or password"): `thincd`'s REST API has **no authentication at all**, and never has — every `thincctl`/web-dashboard write this entire project has ever made succeeded with zero credentials, protected only by network reachability. Separately, the existing SSH-account mechanism for containers (`ldap ssh-target register`, ADR-0093-class work) is itself not real LDAP authentication — `openssh.recipe` builds with `UsePAM no`, and no `libnss_ldap`/`pam_ldap` recipe exists anywhere in this project; `sshd` has never actually spoken LDAP. Instead, `thincd` renders `/etc/passwd`/`/etc/group`/`/etc/shadow`/`authorized_keys` as flat files copied onto a registered container's filesystem on every LDAP user/group mutation. Raised directly by the user once this was understood: "we're engineering, not hacking" — real LDAP throughout, for containers *and* the host, not another rendered-copy mechanism layered on top.
...
**Sessions**: an opaque bearer token, sliding idle timeout (configurable; 0 means every write re-authenticates, no session reuse at all), in-memory only — wiped on every `thincd` restart, matching this project's own "ephemeral unless there's a real reason to persist" precedent for the registry/container state.
...
**LDAP backend** (Part 113): `ldapclient.c` verified standalone against a real local `glauth` process — bind, wrong-password rejection, nonexistent-user rejection, closed-port failure, single- and multi-term search — every case cross-checked against `ldapsearch`/`ldapwhoami`, not just self-consistency. **Live-verified against the real deployed `glauth` servers** (192.168.15.95's `ldap-1`/`ldap-2`): deployed via this project's own self-hosted rebuild path (`pkg hostbuild thinc --deploy`), rebooted, confirmed the box came back on the new build, and a real throwaway LDAP user bound successfully (`resultCode=0`) against both servers directly.
...
- `thincctl`/web-dashboard usage patterns change materially: every write now needs a prior login once an admin-group user exists. Every deploy/verification workflow this project's own development process relies on needs to account for that from this point forward.
