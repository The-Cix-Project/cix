...
- `disk`: `statvfs()` on `g_base_dir` (thinC's own data root) — total/free/avail bytes, the same call `diskformat.c` already uses elsewhere in this codebase.
...
- CLI: `thincctl host-stats` (not `stats`, already `stats NAME` for a container — a bare host-scoped verb, consistent with `swap`/`routes`/`logs`'s own flat top-level naming for other `/v1/system/*` resources).
