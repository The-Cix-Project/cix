# ADR-0105: real freshness tracking for `pkg hostbuild kanxeo --deploy`
...
`kanxeoctl pkg hostbuild kanxeo --build-image=... --deploy` chains three
things: the hostbuild itself (`pkg_hostbuild_start()`), a server-side
follow-on assembly of `kanxeod-root.squashfs` triggered once the hostbuild
reaches `PKG_STATE_INSTALLED` (`spawn_kanxeo_bootroot_assembly()`,
ADR-0057), and finally `POST /system/update --image=<artifact_path>/
kanxeod-root.squashfs`. The CLI's own `--wait` polls the hostbuild's own
state until it leaves `fetching`/`building`; once it reports `installed`,
`--deploy` treated `kanxeod-root.squashfs` existing on disk at the
...
from an earlier round. Only a second, unprompted "kanxeo bootroot
...
ever in flight (it's only ever triggered by the "kanxeo" hostbuild's own
...
to what any package or hostbuild name *means*; "kanxeo" is a plain string
...
when `--deploy` was requested for the "kanxeo" hostbuild specifically --
...
on disk -- proceed to `--image=.../kanxeod-root.squashfs`), or the
assembly is no longer running and never advanced past baseline (a real,
reported failure -- `"kanxeo bootroot assembly did not produce a fresh
...
- `--deploy` for the "kanxeo" hostbuild can no longer silently redeploy a
