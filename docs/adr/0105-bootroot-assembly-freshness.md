# ADR-0105: real freshness tracking for `pkg hostbuild thinc --deploy`
...
`thincctl pkg hostbuild thinc --build-image=... --deploy` chains three
things: the hostbuild itself (`pkg_hostbuild_start()`), a server-side
follow-on assembly of `thincd-root.squashfs` triggered once the hostbuild
reaches `PKG_STATE_INSTALLED` (`spawn_thinc_bootroot_assembly()`,
ADR-0057), and finally `POST /system/update --image=<artifact_path>/
thincd-root.squashfs`. The CLI's own `--wait` polls the hostbuild's own
state until it leaves `fetching`/`building`; once it reports `installed`,
`--deploy` treated `thincd-root.squashfs` existing on disk at the
...
from an earlier round. Only a second, unprompted "thinc bootroot
...
ever in flight (it's only ever triggered by the "thinc" hostbuild's own
...
to what any package or hostbuild name *means*; "thinc" is a plain string
...
when `--deploy` was requested for the "thinc" hostbuild specifically --
...
on disk -- proceed to `--image=.../thincd-root.squashfs`), or the
assembly is no longer running and never advanced past baseline (a real,
reported failure -- `"thinc bootroot assembly did not produce a fresh
...
- `--deploy` for the "thinc" hostbuild can no longer silently redeploy a
