...
2. **Raw disk passthrough.** Disks without a role -- no `state-storage`/`rebuildable-storage`/`log-storage`/`container-storage`/`backup` assignment, no cix-managed filesystem at all -- should still be grantable directly to a container as a raw block device, the same way `--device=` already grants a USB/PCI/GPU/net device. A workload that wants to manage its own filesystem (a NAS app, a database wanting raw block access) shouldn't be forced through cix's own format/mount pipeline first.
...
- A disk with no role at all is no longer a dead end -- it's either cix-managed storage (one of the five roles) or directly usable by a container's own workload, never neither.
