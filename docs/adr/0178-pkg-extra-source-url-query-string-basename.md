...
- Any future recipe using a query-string URL for a non-index-0 `pkg_source` entry (the git-raw-file `?ref=<commit>` pattern `kernel.recipe`/`thinc.recipe` both rely on for their config-file/archive fetches) is now correctly staged under its plain basename.
