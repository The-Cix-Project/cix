...
(`image/src/mkbootroot.c`'s `shelled_bins[]`, Phase C) -- every real Kanxeo
...
built `btrfs-progs.recipe` onto its own `kanxeo-hosttools` image yet
...
A box whose `kanxeo-hosttools` image predates `btrfs-progs.recipe` simply
...
- `POST /v1/disks/{name}/format` can now format a disk btrfs, operator-
  chosen via `fs_type`, on any box whose `kanxeo-hosttools` image includes
...
- Two new library recipes (`libblkid.recipe`, reusing `libuuid.recipe`)
  ...
  not yet verified through a live `kanxeod` `pkg install` round-trip or
