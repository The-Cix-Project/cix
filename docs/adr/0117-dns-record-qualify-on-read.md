...
Found live during task #760's full feature regression sweep on 192.168.15.95, right after ADR-0116's `json.c` fix: `cixctl dns record rm sweeptest` returned `404 no such DNS record` immediately after `cixctl dns record create --name=sweeptest --ip=...` had just succeeded and `dns record ls` showed it present — as `sweeptest.uk.home.arpa`.
...
Live-verified directly against 192.168.15.95: `cixctl dns record create --name=sweeptest --ip=192.168.15.200` followed immediately by `cixctl dns record rm sweeptest` now succeeds (previously 404).
