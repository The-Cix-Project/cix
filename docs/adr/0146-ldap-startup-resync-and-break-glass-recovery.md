...
**Part 2 — a genuine break-glass recovery mechanism**, `image/src/thinc-recover.c`, a new, deliberately tiny (~230 line) standalone binary:

- Boots as its own `init=` target from a **second GRUB menu entry on the same installer media** as `thinc-install` (`image/src/mkinstalleriso.c` now takes a `thinc-recover-bin` argument and stages it as a second `/bin/` binary sharing the installer's already-staged runtime libs, and writes a `"thinC Recovery"` `menuentry` alongside `"thinC Install"`).
- Mounts the *already-installed* system's own real containers partition (`/dev/vda5`, the same hardcoded convention `daemon/src/main.c`'s `CONTAINERS_DEVICE` and `thinc-install.c`'s `BOOT_TIME_DISK_PREFIX` already rely on) read-write.
...
- Deliberately does **not** reuse `thinc-install.c`'s own much larger partition-discovery/formatting machinery, even though that's a form of controlled logic duplication — a break-glass tool's real security property is how small and independently-auditable its own code is, and pulling in the full installer's disk-partitioning surface for a task that needs none of it would work against that, not for it.

**Real, deliberate friction**, matching the user's own "not trivial" bar: reaching this code at all requires hypervisor/physical console access to attach different boot media and force a reboot — something no network-side attacker manipulating `thincd`'s own REST API could ever do, gated or not. The typed confirmation on top means a stray or accidental boot into this entry can't silently disable write-gating either.
...
- `image/src/mkinstalleriso.c`'s argv contract grew from 11 to 12 positional arguments (a new `thinc-recover-bin` inserted after `thinc-install-bin`) — both real call sites (`test/test_installer.c`'s own build-and-boot verification, and `daemon/src/main.c`'s `iso_build_start()` behind `POST /v1/system/iso`) updated to match.
- `pkg/recipes/thinc/v1.4.0/build.sh` (the self-hosted ISO-build path, ADR-0057) is pinned to a git tag that predates `thinc-recover.c`'s existence, so `POST /v1/system/iso` on a box whose "thinc" hostbuild artifact still comes from that old pin will now correctly report a missing-artifact error (`thinc-recover (from a "thinc" hostbuild, ADR-0146)`) rather than silently omit the recovery entry — a real, pre-existing staleness this change surfaces but does not fix; re-pinning that recipe to a current tag is tracked separately (task #852), since it needs a live self-hosted build round-trip to verify, not a blind edit.
...
- This is the second break-glass-style mechanism this project has now built with a "small, independently-auditable, deliberately-not-DRY" posture relative to a larger sibling tool (`thinc-install.c`) — the same justified-duplication precedent, applied to a new case.
