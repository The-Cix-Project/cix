...
- Every container network attachment on a real Kanxeo install has been broken since this project's very first kernel build — not a regression, a gap that was always there and never exercised end-to-end until now.
...
- Deployed via a kernel-only update (`kanxeoctl update --image=... --kernel=...`, both supplied together per the documented one-sided-update footgun, task #671) — the control-plane root was rebuilt locally via `build/mkbootroot` to match what was already running (no `kanxeod` code changes bundled into this deploy), landing on the box's inactive slot A and confirmed booted (`slot: "a"` in `GET /system/boot`) before testing.
