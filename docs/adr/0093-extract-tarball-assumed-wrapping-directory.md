...
- Verified via the existing `test_pkg.c` regression suite (unaffected -- every existing fixture uses the normal wrapping-directory shape, confirming no behavior change there) plus the reasoning above for the git-archive shape, matching what this project's own `thinc.recipe` self-build source snapshot actually produces.
