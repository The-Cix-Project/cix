...
This project's first LDAP deployment (Phase 21, ADR-0036) used `lldap` (Rust + a bundled WASM web frontend) as the LDAP-serving workload. That frontend is dead weight here: Kanxeo's own REST layer was always going to own user/group management end to end, with no human-facing admin UI ever needed against the LDAP server itself. `lldap`'s dependency footprint (a full Rust toolchain, WASM build step) is disproportionate to what this project actually uses it for.

Replacing it opened four real, sequential design questions, tackled as tasks #723-727:

1. **What LDAP server, and how does Kanxeo build it?** (#723/#724)
2. **How does Kanxeo know which running container is the/a LDAP server?** (#725)
3. **How does Kanxeo get user/group data into that server, and keep it current?** (#726)
...
Passwords: `passsha256` (a real, documented glauth "config" datastore field) via kanxeod's own already-linked OpenSSL `libcrypto` SHA-256 — never persisted or echoed as plaintext, and never returned by any `GET` (only a `has_password` boolean is).
...
The delivered credential (`ldap_generate_secret()`: 16 raw bytes from `/dev/urandom`, hex-encoded to 32 characters — a new, minimal, in-process randomness primitive; PKI key generation shells out to `openssl` instead, but a single bind secret doesn't need a real keypair) is written into the container's own filesystem at `<ldap_secret_dir>/bind.secret` (default `/etc/kanxeo-ldap`), chmod `0600`, via `/proc/<pid>/root/` — byte-for-byte the same `persist_mkdir_p()` + `persist_atomic_write()` + `chmod` shape `pki_cert_deliver()` already established for a TLS keypair, just for one secret file. **The plaintext secret is never persisted anywhere in Kanxeo's own state** — only its SHA-256 hash (`passsha256`) survives in the durable user record — so every single fire of this hook, including every `restart:"always"` respawn, generates and re-delivers a fresh secret even though the underlying account may already exist (`LDAP_RECORD_ERR_DUPLICATE` tolerated the same way `pki_issue` already tolerates `PKI_ERR_DUPLICATE` on a respawn's fresh pid).
...
- **Kanxeo owns the durable record; the target server's own config file is a rendered, always-fully-rewritten projection of it**, preserving any operator-authored prefix above the managed `[[users]]`/`[[groups]]` tail — the same split DNS already uses, not a second independently-owned datastore.
