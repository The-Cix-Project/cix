...
Isolated with controlled, before/after tests against a throwaway container (`kanxeo-hosttools`, which has a real `curl`) on the same `management` network:
