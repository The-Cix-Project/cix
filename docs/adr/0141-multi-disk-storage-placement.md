...
Direct follow-up to the disk management web UI (ADR-0140): the user asked where packages/images, Kanxeo's own config, and logs actually live today, and whether another disk role was needed. Investigated the real code rather than guessing, and found every one of the following lives under one single directory tree, `g_base_dir` (`/var/lib/kanxeo`, the OS disk's fixed `containers` partition) -- the *only* exception being `/boot` (the ESP) and one tiny file needed before the daemon itself starts (`/config/net.conf`):
...
- Container workload data is never covered by any of the above -- `state-storage` is Kanxeo's own definition of itself, not what containers are doing.
...
- Every category the user asked about (packages/images, Kanxeo's own config, logs) becomes genuinely relocatable, closing the gap found during this investigation.
...
- CLI: `kanxeoctl storage state|rebuildable|logs show|migrate --disk=NAME` (omit `--disk=` to migrate back to the OS-disk default), `kanxeoctl backup-config show|set|snapshot-now|status`.
