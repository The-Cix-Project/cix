...
The atomic write needs one more `PATH_MAX` of stack in `run_mksquashfs()` and, transiently, room for two roots in `BOOTROOT_DIR` — ~10 MB each, on a partition sized for the platform's rebuildable state.
