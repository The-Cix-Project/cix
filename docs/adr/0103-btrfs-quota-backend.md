...
`f_type` against `CIX_BTRFS_SUPER_MAGIC` (`0x9123683e`). It's the one place
...
`include/linux_compat.h` with a `CIX_` prefix rather than including
...
(`CIX_BTRFS_IOC_SUBVOL_CREATE` = `0x5000940e`, `CIX_BTRFS_IOC_QUOTA_CTL` =
`0xc0109428`, `CIX_BTRFS_IOC_QGROUP_LIMIT` = `0x8030942b`) were hand-derived
...
verified `CIX_FS_IOC_FS{GET,SET}XATTR` constants with the identical method,
