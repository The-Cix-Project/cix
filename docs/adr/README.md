...
| [0132](0132-log-panel-fixes-and-cli-polish.md) | Real-world fixes after the logging epic shipped: log panel fixed-size CSS bug (`min-height: 0`) + batched DOM rendering + generalizing the audit log's `GET /health` exclusion to every `GET` (fixes both the visual growth and the dashboard slowdown, same root cause); dynamic `<instance_name>>` shell prompt; `process ls` restyled to match `ps`; new `container ls` synonym | Accepted |
