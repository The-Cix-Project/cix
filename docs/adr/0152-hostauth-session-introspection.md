...
**CLI**: `kanxeoctl hostauth-sessions ls`, `kanxeoctl hostauth-sessions revoke USERNAME` — a new top-level command, flat-named to match `hostauth-config`'s own existing convention (not nested under a `hostauth <verb>` group, since there's no other `hostauth` noun with subcommands to group alongside).
