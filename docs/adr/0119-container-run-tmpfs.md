...
- `/run` now behaves the same way inside a Kanxeo container as it does on every real Linux host: empty at start, safe for any daemon's pidfile/socket/lock conventions, never a source of stale cross-restart state.
