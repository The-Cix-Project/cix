...
CLI: `kanxeoctl ldap group update --name=NAME --gidnumber=N [--new-name=NEWNAME]`, `kanxeoctl ldap user update --name=NAME [--new-name=NEWNAME] ...` — a distinct flag from `--name=` (which continues to mean "which record"), not a reused/overloaded one, avoiding the exact kind of ambiguity a prior CLI design in this project (`--file=`'s own owner/group fields, caught before shipping) already ran into once.
