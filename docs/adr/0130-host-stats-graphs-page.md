...
- The "all interfaces combined" network chart is honest but coarse — an operator wanting per-interface host-level throughput still needs `kanxeoctl host-stats` or a direct API call and their own filtering; not solved here, and not asked for.
