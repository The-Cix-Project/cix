...
**The image-crossing half of this is not gated by a test that runs.** `test_pkg_build_log` is not in `SELFTESTS` — it creates a real build container, which a composed build container cannot do (#224) — and reproducing the actual bug there would need a second image bootstrapped with its own toolchain, for a lookup rule three lines long. What that test does gate, for anyone who runs it, is that the name-only path resolves and that the 404 names what is building — and "for anyone who runs it" was itself an unmeasured claim, because **that file is compiled by no target any release runs**: the `cix` recipe builds named targets plus `make selftest`, never `make all`. `recipes/package/probe-buildlog-compile/1` settles it. Measured on 192.168.15.95, 2026-09-17: `make build/test_pkg_build_log` compiles and links clean under Cix's own `tcc -Wall -Werror`, and the binary contains `?name=slowbuild`, `?name=nosuchpackage` and `no build in progress`, so the new cases are real code rather than lines in a file nothing reads. They still run in no gate.
