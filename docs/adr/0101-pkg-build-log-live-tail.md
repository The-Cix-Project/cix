...
- `client/src/console.c` gains `kx_pkg_build_log_run()`, sharing `do_ws_handshake()` (generalized from `kx_console_run()`'s own handshake, which previously assumed a `/v1/containers/{name}/console` path) — no raw terminal mode, no stdin relay, just print each frame to stdout. `thincctl pkg build-log` is the new CLI entry point.
