...
- `client/src/console.c` gains `cix_pkg_build_log_run()`, sharing `do_ws_handshake()` (generalized from `cix_console_run()`'s own handshake, which previously assumed a `/v1/containers/{name}/console` path) — no raw terminal mode, no stdin relay, just print each frame to stdout. `cixctl pkg build-log` is the new CLI entry point.

## Consequences

- An operator can now watch a real build's output as it happens (`cixctl pkg build-log`, or the equivalent raw WS connection), not just after the fact.
