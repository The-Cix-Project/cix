...
6. **The corpus filename decides the format; the daemon decides on the content, because it has no filename to go on.** [ADR-0305](0305-a-recipes-format-is-its-filename.md)'s rule holds where recipes live — `<name>@<version>.<ext>`, one format per version, never both. It cannot hold inside the daemon: `image_recipe_path()` stores an image recipe as `<name>.recipe`, one file per image, carrying neither version nor extension. So `parse_image_recipe_buf()` dispatches on a leading `{`, which the legacy form cannot produce — it is comments and a single `image_packages=` assignment. A draft of this ADR said "the extension decides" for both, which would have been a rule the code could not obey.
## Consequences
