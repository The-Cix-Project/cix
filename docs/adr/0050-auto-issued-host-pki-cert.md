...
Direct follow-up in the same identity-focused conversation as ADR-0049 (PKI CA reset): "By default we have a Cert for host.site.domain_suffix?" This is the leaf that Phase E7 (real TLS termination in `thincd`'s own listener, planned separately, not part of this phase) will eventually serve — building it now, correct and already synced with site config, rather than inventing it under time pressure inside that larger, later phase.
...
- No consumer of this cert exists yet -- `thincd`'s own HTTP listener still serves plain HTTP only (Phase E7, not part of this work). This leaf sits correct and ready, genuinely unused until that lands.
