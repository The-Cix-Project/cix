...
- This closes the diagnostic half of the real gap that blocked live diagnosis of a real `lldap` build failure on 192.168.15.95 — the box itself still needed a locally-rebuilt-and-pushed `thincd` to pick this fix up (no way to hot-patch a running daemon), and the original failure's own root cause was never captured by the old mechanism, so it still had to be reproduced separately to find.
