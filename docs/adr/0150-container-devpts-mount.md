...
- `container_dev_bpf_attach()`'s own cgroup device policy is unaffected: it's only ever loaded when a container specifies explicit `--device=` entries (`device_count > 0`); a container with none (every container in this fleet so far) gets no Cix-authored `BPF_CGROUP_DEVICE` program at all, so nothing here needed a device-cgroup allowlist change — confirmed by reading `container_dev.c` before assuming otherwise.
