...
**`net.conf` stays the boot source of truth and gains a second writer, so the format moves into one module.**

> **Correction, 2026-09-13.** This ADR's Context said `net.conf` *was* the boot source of truth for the management address, on the strength of `NET_CONF_PATH` appearing at exactly two sites. That was false when written. The address is recorded in a third place — the boot loader entry's `--bind=<ip>`, written by `cix-install.c` and regenerated from `g_bind_addr` by the A/B update path — and `cixd`'s listener bound *that*, not `net.conf`'s value, because `bootstrap_management_network()` updates the global `g_bind_addr` while `main()` passed its own argv-derived local to the listener. So this endpoint changed `net.conf` and the registry, the stale `--bind=` won at the next boot, the daemon could not bind an address no longer on the bridge, and the box was lost. The grep that would have found it was `--bind=`, not `net.conf`. [ADR-0284](0284-cixd-always-binds-loopback.md) records the fix and the design the owner chose in response; `net.conf` is genuinely authoritative only from that change onwards. `daemon/src/netconf.c` holds `netconf_parse()` and `netconf_write()`; `cix-install` and `cixd` both link it. Before this, the format was defined twice — a `snprintf()` in the installer and a static parser in the daemon — which was survivable only while there was exactly one writer. Adding a second writer without unifying the format would have put two independent definitions of one file in two programs, which is the One Source of Truth failure the maxims forbid. The write is atomic (temp file, `fsync`, `rename`) because the entire purpose of the file is to be read on the next boot, and a half-written one is a box that does not come back.
