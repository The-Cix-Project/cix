...
| GET | `/system/boot-manager` | The ESP's installed boot manager binary -- size, sha256, whether a backup exists (#469) |
...
| POST | `/system/interfaces/{name}/flap` | Flap a host NIC (down then up) to recover a stuck link -- flap-only, so an interface can never be left down |
...
| GET | `/deployments/{name}` | One deployment's full detail, including its raw, unsubstituted text |
| DELETE | `/deployments/{name}` | Remove a deployment |
| POST | `/deployments/{name}/apply` | Render `{name}`'s own stored recipe (substituting any `{{SECRET:KEY}}` tokens from the request's own `secrets` object, and `{{LDAP:URI}}`/`{{LDAP:BASE_DN}}`/`{{LDAP:BIND_DN}}`/`{{LDAP:BIND_PASSWORD}}` tokens from the daemon's own LDAP client config, issue #66 — no per-apply secret, no copied values. `{{LDAP:URI}}` resolves to the same **effective** list the daemon's own `nslcd.conf` rendering uses — the registered servers' live addresses unless an explicit `client_uri` overrides them, health-filtered either way — rather than to the raw stored field, so a deployment that never typed a copy of its own server list still renders a usable URI (#327)) and create the container from it (201). **If the image it names exists but has not been built yet, this returns `202` and waits for it rather than failing** — ADR-0270, see below |
...
| GET | `/storage-roles` | List persisted disk role assignments |
| POST | `/storage-roles` | Assign a role (container-storage/backup/rebuildable-storage/log-storage/swap) to a disk or partition |
| DELETE | `/storage-roles/{disk_name}` | Remove a disk's role assignment |
...
| GET | `/images/{name}/versions/{version}/manifest` | What one specific image **version** holds — the `name@version` pairs installed into it, snapshotted when it was produced (#398) | The image-level manifest is a *live* document. A container records the version it runs from and keeps running it, so "what is in this container" must be answered from the version — reading the live manifest describes what the image holds *now*, a different question with the same shape. Deliberately not the *declared* manifest: that is live intent, and a `rolling` entry's version there is a floor rather than a fact. A version produced before snapshots existed returns 404 rather than falling back, deliberately: the fallback would be a wrong answer with nothing marking it. And an installed record is still not proof the bytes reached any container (#395). |
...
| GET | `/pkg/recipes/{name}` | One recipe version's full detail, including its raw recipe text, its `language` (`shell`/`cbs`) and its `artifact_format` (`cixpkg`/`tar.gz`, [ADR-0307](../adr/0307-a-packages-artifact-format-is-the-one-its-recipe-declares.md)); `?version=` selects a specific one, omitted resolves to the highest available |
...
| GET | `/pkg/verify` | Every installed package whose own recorded files are **not present** in its image's current rootfs (#281), or whose installed headers make includes that do not resolve (#289) |
...
| POST | `/pkg/{name}/artifact/publish` | Publish an already-built artifact to the configured artifact server without rebuilding it (issue #171) -- builds the tarball from the installed tree first when a hostbuild's doesn't exist yet. Async (202); with a name installed in several images it publishes the hostbuild entry (#520) |
...
**Seeing whether a shipped header can actually be included.** A package that installs headers is shipping an interface, and an interface that cannot be included is broken whether or not anything uses it yet. `linux-pam` shipped exactly that for six revisions: it installed `security/pam_misc.h`, whose very first include is `security/pam_client.h`, and never built the `libpamc` directory that header comes from. Nothing linked `pam_misc`, so nothing noticed — until util-linux ran a compile test on it while packaging `login(1)`, concluded PAM was unusable, and refused to build. The error named PAM, not the missing header, and not `linux-pam` (#289).

This recipe set builds directory-by-directory on purpose, so any recipe installing *some* of an upstream project's headers can carry the same defect silently. The check is the cheap half of what a compiler does: for each installed header, does every **unconditional, same-project** angle-bracket include resolve to a file in this image's include tree? Angle-bracket only (a quoted include follows different rules); unconditional only (an include inside `#if`/`#ifdef` is often meant not to resolve here, with a file's own include guard deliberately not counted as one, or nothing in a guarded header would be checked at all); **same-project only** — the include must sit in the same directory under `usr/include/` as the header making it; and existence rather than compilability (whether the resolved header itself parses is the compiler's question).

That last limit was added after measuring, not before. The unrestricted form flagged 48 of 202 real installed packages, essentially all of them `<stddef.h>` and `<stdarg.h>` — **freestanding** headers the compiler provides from its own directory rather than `/usr/include`, so they never resolve there and are never a defect. Same-directory is the exact shape of the real failure (`pam_misc.h` reaching for `pam_client.h`, both under `security/`) and is what a *recipe* is answerable for, since a project's own headers are the ones its own build either installs or does not. What it gives up is a header reaching into another *package* that is not installed here — a dependency question, and an image-dependent one.

It is **reported, never fatal to an install** — a deliberate difference from the ELF undefined-symbol gate, which refuses. That check has essentially no false positives; reading C without a preprocessor cannot make the same claim, so this one says so loudly instead: in the daemon log at install time, and here afterwards.

It is deliberately its own endpoint rather than a field on `GET /pkg`: it stats every file of every installed package, and `GET /pkg` is polled every two seconds by the dashboard and is already this daemon's single largest source of event-loop stalls. The cost is measured rather than estimated — 2.66 s for 202 installed packages on the reference host, which `GET /system/stalls` attributes as that boot's worst loop pass, by name. Below the 5 s stall threshold and above the 750 ms slow-pass one: about one dashboard poll's worth of blocked control plane, which is fine for a verb an operator runs and is exactly why it is not on a polled path.
...
The `--console=NAME=/path args` form splits the command on spaces, so an argument containing a literal space cannot be written that way — a real limit, and the reason a deployment (`recipes/deployment/<name>/*/container.json`, a real JSON array with nothing to lose in quoting) is the better place to declare anything non-trivial.
...
- **Five operations that used to turn it off silently are refused with `409`:** deleting the group named in `admin_groups`; changing that group's `gidnumber` (members carry gids and nothing rewrites theirs when the group's own gid moves, so the group empties); deleting the last admin user; disabling or de-admining them; and pointing `admin_groups` at a group nobody is in. **Renaming an admin group is explicitly allowed** — [ADR-0147](../adr/0147-ldap-user-group-rename.md) already rewrites `admin_groups` in place, so a rename never drops it out of gating. Two are narrower than they sound — **only the last admin is protected** (while another remains, removing one is ordinary), and **only turning ACTIVE gating off is refused**, so setting `admin_groups` before the admin users exist still works and remains the normal way to enable gating for the first time.
...
**Recovering a stuck NIC: `POST /system/interfaces/{name}/flap`.** A physical interface can end up with a link but no connectivity — a carrier that came up in a bad state, or a bridge port stuck in `blocking`. `POST /system/interfaces/eth0/flap` brings the interface administratively **down and then straight back up** over rtnetlink, one atomic action that renegotiates the carrier. It is *flap-only* by design — there is no bare "set the interface down", because a down-only call is how an operator strands a box on the very interface they are trying to fix. Run it from the console (which talks to `127.0.0.1` and is unaffected); run it against an off-box address that rides the interface being flapped and the reply may not come back even though the flap succeeded — the interface is momentarily down — so reconnect and re-check rather than reading a dropped reply as a failure. `cixctl network flap-interface eth0` is the client for it.
...
```json
[{"ts": 1700000012, "source": "audit", "level": "info", "container": "", "msg": "alice POST /v1/networks"}]
```
...
Forget-on-delete stays correct — nothing dangles — and `POST /v1/deployments/{name}/apply` now restores a working server rather than half of one.
...
- `ready` is exactly one of `tcp_port`, `socket` (a unix socket path) or `command` (an argv that exits 0, run as the service's uid, 5 s timeout, output discarded), with `timeout_seconds` (default 30). A probe that never passes is reported as a failure and the service is treated as ready so dependents proceed — the behaviour the old container-level check had.
- A `tcp_port` probe connects on **any** of the container's own addresses: `127.0.0.1` first, then each attached network's address in declaration order, one candidate per supervision turn until one answers ([#477](https://git.home.arpa/itdlabs/cix/issues/477), [ADR-0298](../adr/0298-a-readiness-probe-tries-every-address-the-container-has.md)). There is no single right address — “listening” covers a service bound to `0.0.0.0`, one bound to loopback only, and one bound to exactly one interface of a multi-network container. Until #477 this probe only ever tried `127.0.0.1`, whatever addresses the container had, so a service binding only its network address could never be reported ready.
...
- `disk` is optional (task #638, [ADR-0102](../adr/0102-per-container-disk-selection.md)): a bare disk name (e.g. `"sdb"`, from `GET /disks`) to place this container's own writable storage on, instead of the default OS disk. The disk must already be mounted and carry the `"container-storage"` role (`POST /storage-roles`) — `400` if it doesn't exist, isn't mounted, or lacks that role. Echoed back as `disk` on `GET /containers` (`null` for the default placement).
...
- An attachment may also carry `ifname` ([ADR-0259](../adr/0259-an-interface-may-be-named.md)) — what that interface is called **inside** the container: `{"name": "management", "ip": "192.168.15.101", "ifname": "management"}`. Omit it and the container gets the positional `eth0`, `eth1`, … it always did, so leaving it out changes nothing. The name is 1–15 characters (`IFNAMSIZ - 1`) of `[A-Za-z0-9._-]`; `lo` is refused, and so is any `eth<digits>` name, since that is the namespace unnamed attachments are assigned from — refusing the prefix is a rule instead of a collision matrix. Two attachments on one container may not share a name (`400` at create, `409` on a live attach). The same field works on `POST /containers/{name}/networks`, and `cixctl container network attach NAME --network=NET --ifname=NAME` passes it.
- An attachment may instead be a **port of a bridge inside the container** ([ADR-0264](../adr/0264-a-container-may-bridge-its-own-interfaces.md)) — `{"name": "access", "ip": "192.168.151.1", "container_bridge": "br0"}`. Cix creates `br0` in the container's own netns at creation time, before any service starts, and enslaves the veth to it; the address goes on the **bridge**, never on the port, because a bridge port with an address of its own does not receive on it. Several attachments may name the same bridge — the first creates it, the rest join. Same name rules as `ifname`, since it becomes a real interface in the same namespace.

  This exists for access points, where the two halves of the bridge arrive from opposite directions. `hostapd` owns the radio and puts it into whatever bridge its own `bridge=` names, creating that bridge if it has to — but it knows nothing about the container's other interfaces and will never enslave one. Cix owns the wired half. Building the bridge first means `hostapd` finds it and adds the radio, so wireless clients reach the wired segment. The alternative — a service inside the container running `ip link` — would need iproute2 in the image, a shell to drive it, and a network the platform did not build.

  **Creation-time only.** `POST /containers/{name}/networks` refuses it with a `400`: building a bridge means acting inside the container's network namespace, which the live-attach path has no primitive for. Saying so beats accepting the field and ignoring it.
- `procfuse` is optional, default `true` ([ADR-0262](../adr/0262-a-container-sees-its-own-limits.md), issue #336) — whether this container's `/proc/meminfo`, `cpuinfo`, `stat`, `uptime`, `loadavg` and `swaps` report **its own cgroup's** limits instead of the host's. One host-side FUSE server answers every container, identifying the reader from the request's own pid, so `free`, `nproc`, a JVM sizing its heap or `make -j$(nproc)` see the container. Default on, because a virtualised `/proc` a recipe has to remember to ask for is useless — nobody knows they need it until something has already sized itself wrong, which is #278/#279 exactly: a build sizing itself by host memory against a 2 GiB ceiling it could not see, surfacing as an unexplained OOM against a machine reporting plenty free. `false` declines it. Silently a no-op when the host's server is not running, in which case the container reads the host's real `/proc` — degraded, never a reason to refuse to start a container. `cpuinfo` follows `cpuset.cpus.effective` and **not** the CPU quota, which is measured rather than chosen: a build job's `cpu.max` is one CPU while `nproc` reports 2, and `nproc` reads `sched_getaffinity`, which a cpuset constrains and a quota does not — a quota-derived `cpuinfo` would turn every `make -j$(nproc)` on this platform into `-j1`. Not a security boundary: it makes tools size themselves correctly, it does not hide the host.

  Nine files, not six: `/proc/{meminfo,cpuinfo,stat,uptime,loadavg,swaps}` plus `/sys/devices/system/cpu/{online,present,possible}`. The sysfs trio is there because a container's CPU count has two independent answers — `sysconf(_SC_NPROCESSORS_ONLN)`, which a great many libraries size a thread pool from, reads `/sys`, not `/proc`. A container pinned to one CPU on a two-CPU host measured `CONF=2 ONLN=2 NPROC=1`: `nproc` was right (it uses `sched_getaffinity`, which a cpuset constrains) and every ONLN-based sizing decision was wrong by a factor of two. All three report the container's cpuset verbatim, which is already a kernel range list and is the same set `/proc/stat`'s per-cpu lines are filtered to. **What this does not fix:** `_SC_NPROCESSORS_CONF` counts `cpuN` *directories*, and no file bind removes a directory — so `htop`, which takes its meter count from that, still draws a meter for a CPU the container cannot use and marks it offline (#363). A private sysfs mount does not help: every container already gets one (`src/mountns.c`), and CPU topology is not scoped by any Linux namespace.

  `GET /containers/{name}` reports what actually happened, as `procfuse_expected` and `procfuse_bound`. The bind is deliberately non-fatal and the runtime's own failure text goes to the container's stderr, where nothing collects it, so the daemon counts the binds in the container's own `/proc/<pid>/mountinfo` instead of assuming the request took effect. That count is taken **live, each time you ask**, not stored at creation: the child is still building its mount table when the create call returns, so a count taken then reads a half-built namespace and reports a fault that is not there. `null` means the table could not be read (not running) and is deliberately distinct from `0`. `procfuse_bound < procfuse_expected` means the container is running and reading the **host's** real `/proc` for the missing files, while still reporting `status: running`. `procfuse_expected` of 0 means it declined with `procfuse: false`, or the server was not running when it was created; neither is a fault.
...
A backup keys a shell recipe `<name>/<version>` exactly as it always has, and a CBS recipe `<name>/<version>/build.cbs`. The asymmetry is on purpose: an older daemon rejects a key it cannot read as `<name>/<version>` — and rejects the whole document when it does — so making every key three segments would cost every recipe in a backup restored onto the other boot slot. A restored `build.cbs` has its identity **re-derived** rather than restored, since `explain.json` is what one particular `cbs` made of the document.

Two fields used to have no CPDL home, and this paragraph used to say so: *"`changelog` is always `null`, and a CBS recipe cannot carry an artifact checksum, so it rebuilds from source instead of taking a cache hit."* **Neither is true any more.** cix-build-system#161 closed, a `build.cbs` carries both in its opaque `metadata { }` block, and `parse_cbs_recipe()` reads them from there — so a converted recipe takes a cache hit exactly as its shell revision did, and reports its changelog.
...
| key | what it is |
|---|---|
| `language` | `shell` or `cbs` — which language the recipe is WRITTEN in. It is the filename, so it cannot disagree with what runs. Called `format` until ADR-0307; a clean rename, no alias |
| `artifact_format` | `cixpkg` or `tar.gz` — what the version PUBLISHES. A CBS recipe declares it; a shell recipe has no field and is `tar.gz` by construction |

The two are not the same question, and the daemon **reads** the declared one rather than inferring it from the language. CPDL accepts `format "tar.gz"` and `cbs build` then refuses to execute it (*"standalone builds require cixpkg"*), so a CBS recipe declaring `tar.gz` is **refused at publish** with a message saying so — storing it would mean an immutable version that fails at build time, every time. Assuming `cixpkg` for every CBS recipe would accept that recipe happily, and would bake in a restriction that is CBS's to lift rather than this platform's to encode.
...
Four stages are new. **`acquire`** is where a pipeline waits for what it depends on, and is where forking attaches — a deployment waiting for its image to be built sits exactly there (ADR-0270). **`assemble`** and **`stage`** are host-only and fail independently of each other (`GET /system/assembly` has its own generation counter; `POST /system/update` answers `{"status":"staged","slot":...}`). These two race by construction: assembly runs *after* the hostbuild reports `installed`, so a client acting on that status can call `/system/update` while the squashfs is still being written. That case now answers `409` naming the in-flight assembly generation (poll `GET /system/assembly` until `running:false` and retry), not the misleading `400 "image_path is not a squashfs image"` that reads as a wrong path or a corrupt build (#435). **`verify`** is deployment-only and is the ready probe — a container that started and is not ready is neither ok nor failed, which is exactly how a crash-looping dnsmasq reported `"status":"running"` and told nobody anything.
...
## Gates — holding automation where a change escapes its blast radius (ADR-0273)

Three things happen on this platform without anyone asking, and each is a point where a change stops being local:

| Gate | Holds | Because |
|---|---|---|
| `publish` | an artifact before it is uploaded to the shared cache | every host that installs it inherits those bytes, checksum-verified and trusted |
| `roll` | a rolling rebuild before an image moves | the image moves to a version **nobody asked for** — a recipe was published and the drain converges |
| `deploy` | `POST /system/update` before it writes a boot slot | the next reboot runs it |

**All three default to `false`, and a host with them off behaves exactly as it did before they existed** — not approximately: the hold is one test at the head of each drain and one refusal in the update handler.

There are three because there are three escape points, not because three is tidy. Ten of the eleven pipeline stages hold a change inside the blast radius it already has, where there is nobody to ask.
...
**A held item never starves what is behind it.** The rebuild drain walks by index and skips a held image rather than stopping at the front; the push queue — which removes an entry as it takes it — steps over held ones while choosing. A gate on one thing must not become an outage for everything.

**This is not four-eyes.** The same identity may request a change and approve it, so a gate constrains *automation*, not a person. That is still worth having: the caller being held today is a token-bearing script (`pkg hostbuild --deploy`, or an agent), and making it stop for a deliberate second call is the pause being bought. Requiring approver ≠ requester needs roles — [#304](https://git.home.arpa/itdlabs/cix/issues/304).

Every approval writes an audit line naming the user, the gate and the target ([ADR-0271](../adr/0271-an-action-is-attributable-to-a-person.md)). That record is the point.
...
- **`probe`** &mdash; how the verdict was reached, reported so `healthy` is never read as more than it is. `tcp:PORT` is a real service check (the server accepted a connection). `process` means only that the providing container is running &mdash; used for the UDP services (NTP, syslog), where a TCP connect would be a meaningless check dressed up as a real one. The LDAP probe's `PORT` **follows `client_tls`** (#416): it reads the same `ldap_client_port()` the client URI does, so the probe can never test a port the clients were not given. It was fixed at 3893 until `ldap-1`/`ldap-2` 1.6.0 turned that listener off, at which point both servers reported `connect: Connection refused` against a deliberately closed door &mdash; clients kept working via the unfiltered-list fallback below, so the only symptom was a health view that was simply wrong.
...
- **`client_tls` on `PUT /v1/ldap/config`** (#414) configures the clients *inside containers* — `nslcd`, `ldapsearch`, anything handed the `{{LDAP:URI}}` token. With `client_tls: true`, `effective_client_uri` names `ldaps://<ip>:<server_tls_port>` instead of `ldap://<ip>:<server_plaintext_port>`, and the certificate the server presents carries the container's IP as an `IP:` SAN, not just its name — a TLS client checks the name it *dialled* against the matching SAN type, so a server reached by address needs an address SAN or verification fails against an otherwise-correct certificate. There is no separate `client_tls_port`: #419 removed it, because there is one port per listener and the port clients get is the server's own. `client_tls` now selects *which enabled listener* clients are pointed at, and naming a disabled one is a 400 rather than a URI nothing answers.
...
- **`server_plaintext` / `server_tls` (+ their ports) on `PUT /v1/ldap/config`** (#419, [ADR-0282](../adr/0282-glauths-listeners-are-configuration.md)) configure the *servers* — rendered straight into each registered glauth's own config file as `[ldap]`/`[ldaps]` `enabled` and `listen`, The value is also applied to what is **staged** for that container on its next start, before `clone3()` — glauth binds its listeners at startup and its config watcher reloads only the record datastore, so a value written into a live config is never adopted, and without the staging half a restart would re-stage the recipe's own value over it. Measured on v2.57.114 before that half existed: `[ldap] enabled` read true in both live configs while 3893 stayed refused, and a real stop/start did not change it. So a listener change reaches a **running** server when that server restarts — `cixctl ldap config set --restart-servers` does that one at a time, each back before the next is touched, and is off by default because this is the directory that authenticates the control plane. Until the first PUT carrying one of these, `listeners_managed` is `false` and the daemon writes neither section, leaving each server's own config in charge — the same posture the `baseDN` rewrite takes before `ldap_base_dn` is ever set, and for the same reason: a default derived from nothing would rewrite a live listener on the first boot after upgrade. Both listeners off is a 400; turning `server_tls` on while a registered running server has no delivered certificate is a 409 naming that container, because glauth exits on reload if told to serve TLS without one.
...
- `workload_memory` is the `cix-workload` parent cgroup's own memory — every container and package build this daemon starts lives under it (ADR-0165) — reported beside the host's because **both numbers are true at once and only the host's was ever visible**. That is how a live OOM presented as a healthy, mostly-idle machine: 5.88 GB available on the host while `cix-workload/cix-pkgbuild` sat pinned at its 2 GiB ceiling, OOM-killing continuously ([#279](https://git.home.arpa/itdlabs/cix/issues/279)). `usage_bytes` against `limit_bytes` is the pair that answers "is something at its ceiling"; `pressure` is what distinguishes a workload merely near its limit from one being hurt by it. `limit_bytes` is `-1` when the parent is unlimited (the default) rather than `null`, because `null` would read as "unknown" when it means "no ceiling to hit". The whole object is **absent** on a host that has never started a workload — there is no ceiling to report then, and zeroes would read as a workload pinned at nothing. The dashboard charts it against its own limit rather than host total, since a build pinned at 2 GiB is a flat line near the bottom of an 8 GiB axis, which is precisely what the host memory chart looked like while that build was dying.
...
**The status stays a uniform `404`; the message no longer does.** Both calls used to capture modprobe's stdout and stderr into a pipe and discard it, so a real refusal on a live box read `{"error":"modprobe -r could not unload this module"}` and named nothing -- while modprobe itself had written the answer (`FATAL: Module usb_storage is in use.`, `is builtin.`, `not found in directory /lib/modules/...`), each pointing at a different fix. Quoting the tool verbatim is not the same as parsing it for a status code, which is what the uniform `404` was actually protecting against, so the message now carries whatever modprobe said. The same applies to `autoload` at boot, which additionally reports to the log store rather than stderr -- boot-time stderr goes nowhere an operator can read (#132), so an autoload that silently did not happen was indistinguishable from one never configured.

**`POST` with `options` on an already-loaded module is `409`, not `200`.** `modprobe` on a loaded module succeeds and does nothing, so the endpoint used to answer `200` and echo back parameters the kernel had never seen -- a reported success for work that did not happen. Module parameters are set when a module is inserted; changing them means unloading first, and whether unloading a live module is acceptable is the operator's call rather than the daemon's, so this refuses instead of deciding for them. A `POST` with no `options` on something already loaded is still `200` -- idempotent, and claiming nothing untrue.
...
{"config_symbols": ["CONFIG_DUMMY"]}
```

For a driver that isn't already in this platform's own curated `=m` module set, but does live in mainline Linux (the overwhelmingly common case) -- an ordinary hostbuild against the `kernel` recipe itself, the exact same mechanism `POST /pkg/hostbuild` drives (same shared pool of pkg-build chain slots, same `409`/`PkgEntry` response shape, progress polled the identical way via `GET /pkg/hostbuild/kernel`), gaining only an optional `config_symbols` list. Each entry (a bare `CONFIG_*` name) is merged into the same curated kernel config this platform already builds from, forced to `=m`, via a second `merge_config.sh` fragment -- strictly additive: an empty or omitted list reproduces the exact existing kernel build unchanged. The build environment is composed from the `kernel` recipe's own `pkg_build_depends=` like any other build ([ADR-0304](../adr/0304-a-hostbuild-composes-its-build-environment-like-every-other-build.md)) — there is no `build_image` to name, and a GCC toolchain and `kmod` are present because the recipe declares them (see [`kernel-build-and-ab-updates.md`](../guides/kernel-build-and-ab-updates.md)).
...
```json
{
  ...
  "disk": {"usage": {"bytes": 4096, "measured_at": 1789470000, "source": "qgroup"}, "read_bytes": 0, "write_bytes": 16384, "read_ios": 0, "write_ios": 4, "pressure": {"some": {"avg10": 0, "avg60": 0, "avg300": 0, "total_usec": 0}, "full": {"avg10": 0, "avg60": 0, "avg300": 0, "total_usec": 0}}},
  "networks": [{"name": "internal", "rx_bytes": 1024, "tx_bytes": 2048, "rx_packets": 12, "tx_packets": 9}]
}
```

- `cpu`/`disk.read_*`/`disk.write_*`/`networks[].*_bytes`/`networks[].*_packets` are cumulative counters (since the container started); `memory.*`/`disk.usage.bytes` are gauges (current value, not a delta). **`disk.usage.bytes` has two possible sources and `disk.usage.source` says which answered — read it before using the number** (#475). The three were flat `disk.upper_*` until [ADR-0301](../adr/0301-a-containers-disk-usage-is-named-for-the-question-it-answers.md): `upper_` named an overlayfs upperdir over the image as a lower layer, and ADR-0207 replaced that mechanism with a subvolume *seeded* from the image, so the prefix described something that no longer existed. Nested rather than flattened to `bytes`/`source`/`measured_at` because `disk` also carries `read_bytes` and `write_bytes`, beside which a bare `bytes` reads as their total — and `usage` is deliberately the same name and shape as [`GET /volumes/{name}/usage`](#volumes), which answers the same question about a different resource. This document used to describe the field as the diff from the container's image, and containers built from the same image disproved it: two on 192.168.15.95 both read exactly `29360580`, which a real per-container diff cannot do. On the btrfs substrate a container's writable tree is a subvolume *seeded* from its image (ADR-0207) rather than an overlay upperdir over it, so a walk of that tree sums the whole rootfs.

    - **`usage.source: "qgroup"`** — the writable subvolume's **exclusive allocated extents**: what this container has written that its image snapshot does not hold, which is the number the field always promised. Same accounting and same preference order as [`GET /volumes/{name}/usage`](#volumes), so two endpoints answering "how big is this tree" agree on what counts. One ioctl, so it is read live on every request and forks nothing — but it settles at btrfs transaction commit, so it can be ~30 s behind, and `usage.measured_at` is when it was *read*.
    - **`usage.source: "walk"`** — the summed content of the whole writable tree, which on the seeded-subvolume layout includes the image content. It is the only figure available where no qgroup answers (storage not on btrfs, quotas disabled, or a writable tree that is not a subvolume), and it is now *labelled* rather than passed off as a diff. On this path the figure is **measured in the background, not while answering the request** (#474): the walk is `nftw()`, O(files) and unbounded, and it used to run inline on every stats request, on a pid-1 daemon whose one event loop is the only way into the host. A request serves the last figure and starts a new measurement when that one has aged out, so `usage.bytes` is **`null` until the first measurement lands** (normally the request after the first). Reporting `0` for "not measured yet" was the alternative and it is a lie a caller cannot detect.
...
A real, immediate `SIGKILL` — no grace period, unlike `DELETE /containers/{name}` (which has real container-lifecycle semantics: the container's own restart policy, network/cgroup teardown, etc.). Refuses `pid 1` outright (`400`): on an installed host `cixd` *is* `pid 1`, and killing init crashes the machine.

**This daemon's own pid is refused too** (`400`), whatever number it carries. On an installed host that is `pid 1` and the rule above already covers it; a `cixd` started by hand is refused for the same reason, since nothing would bring it back and the caller would be left with no control plane and no way to say so. ADR-0246 briefly made this killable-when-supervised, and ADR-0247 removed the supervisor along with that allowance. Every other pid is allowed, **including one that happens to belong to a running container** — killing a container's own init pid this way is exactly equivalent to that container crashing on its own; the existing `SIGCHLD`-driven exit handling already covers it correctly, restart policy and all. `404` for a pid that isn't currently running, `400` for a non-numeric path segment. `cixctl process ls`/`process kill PID` is the CLI surface.
...
ADR-0157 Phase 2: several builds can genuinely be in flight at once, up to `pkg-build-config`'s own `max_concurrent_jobs` (10 by default further). Optional `?name=`/`?image=` query params pick which one to attach to — omitting `name` falls back to "the one build in progress" when that's unambiguous (zero or one running); with two builds running at once, omitting `name` gets a `400` asking for an explicit one rather than guessing which the caller meant.

**`?image=` is not defaulted to `base` here, and that is the one place this endpoint deliberately differs from every image-optional endpoint that installs something** ([#476](https://git.home.arpa/itdlabs/cix/issues/476)). A `name` with no `image` means "whichever image is building that package" — the caller is asking about a job that already exists and already chose its own target, so there is nothing to default. Defaulting it made a running host build unreachable by name, because a host build's job is filed under the internal `__hostbuild` image an operator has no reason to know: `cixctl pkg build-log --name=cix` answered `404 no build in progress` for the whole of a running `cix` host build. If two images really are building the same package at once, a bare `name` gets a `400` naming `image` as the way to disambiguate rather than attaching to an arbitrary one.

The two `404`s are also distinct now, because they mean different things to an operator. When nothing matching is building, the message names the builds that *are* — so a lookup miss can never be reported as "there is no build" while one is running. When the build exists but has no live output to attach to at that instant (during the source fetch, between chain steps, or while the artifact is finalized, all states in which no build container exists) the message says to retry shortly.
...
- `/etc/nsswitch.conf`: `passwd`/`group`/`shadow: files ldap` — routes `getpwnam()`/`getgrnam()` through `nslcd`'s NSS module for any account not already in the container's own local `/etc/passwd` — plus `hosts: files dns`, the same line every image's baseline carries ([ADR-0296](../adr/0296-the-platform-keeps-nsswitch-correct.md)). This file **replaces** the image baseline, so until [#478](https://git.home.arpa/itdlabs/cix/issues/478) it silently changed host resolution as a side effect of an LDAP feature: it carried no `hosts` line at all, and glibc's own compiled-in default supplied one. Both variants are now generated from a single definition in `daemon/src/nsswitch.c`.
...
- `devices` is optional: 0–N entries, each a discovered device id from `GET /v1/devices` (`"pci:..."`, `"usb:..."`, `"gpu:N"` for a whole GPU — `gpu:N` is never itself listed by `GET /v1/devices`, only its individual member nodes are — or `"disk:<name>"` for a raw whole disk, ADR-0142), **or** the name of a persistent device mapping (below). Real `/dev` nodes are granted via a `BPF_CGROUP_DEVICE` program on the container's own cgroup (ADR-0017) — nothing else on the host can reach them once bound. A bare `gpu:N` id expands into every node that physical GPU needs in one grant (DRM `cardN`/`renderDN` plus the shared `/dev/kfd` compute node) — see ADR-0028/ADR-0029. A `"disk:<name>"` entry is any non-OS, role-less whole disk `GET /disks` reports — fresh, unlabeled media straight from a USB enclosure or a PCI-passthrough disk with no filesystem/mount concept of its own, handed to the container as a raw block device; a disk already carrying a role (`POST /storage-roles`) is never listed here, since it's already owned by this daemon's own storage-placement system.
- `interfaces` is optional: 0–N real host network interface names (e.g. `"eth1"`) moved directly into the container's own netns (not a veth pair) — fd-anchored teardown, correct even if the container crashes mid-move. See [ADR-0022](../adr/0022-real-nic-passthrough-netns-move.md).

  **This is not the same mechanism as `devices` above, and a wireless adapter deliberately uses this one.** `devices` grants a `/dev` node through a `BPF_CGROUP_DEVICE` program on the container's cgroup; `interfaces` moves a live kernel netdev between namespaces. By the time a container exists, a WiFi adapter's USB endpoint has already been claimed by its driver on the host and turned into `wlan0` — so handing the container the raw USB device would grant it something its driver has already taken, while what `hostapd` actually drives is the interface. `ar-1`'s recipe therefore says `"interfaces": ["wlan0"]` and grants no USB device at all.

  **A radio moves as a whole PHY, not as an interface** ([ADR-0265](../adr/0265-a-radio-moves-as-a-phy-not-as-an-interface.md), issue #341). `rtnl_link_set_netns_pid()` on a wireless interface returns `EINVAL`: the netdev belongs to a wiphy, one wiphy can own several netdevs, and the kernel refuses to let one leave alone. Cix detects this over netlink (never sysfs — a sysfs instance is bound to the namespace it was *mounted* in, so it answers about the host from inside a container, which stranded a radio for real in #345) and uses `NL80211_CMD_SET_WIPHY_NETNS` instead. Everything on that wiphy goes with it. The interface is brought **down** before the move, because `nl80211.h` requires it, and brought back **up** afterwards from inside the target namespace by a forked `setns()` helper — the kernel administratively downs a link as part of moving it either way, so "up" always belongs after the move.

  **The grant is exclusive, and nothing enforces that because nothing has to.** `GET /v1/devices` is the one source of truth for which interfaces exist and are assignable, and an interface already moved into a running container's netns *does not appear there at all* — so a second container asking for it gets `400 unknown or unassignable interface` from the ordinary lookup, with no separate "already claimed" table. While `ar-1` runs, the host has no `wlan0` and cannot use that radio for anything; `DELETE`ing the container moves the PHY back and the host has it again within about a second. An interface merely enslaved to a host bridge stays visible but reports `assignable: false`, read from the kernel's own `master` symlink rather than from a second bookkeeping table.
...
Removes one partition — every other partition on the disk is untouched. `404` if `partition_name` doesn't currently exist or doesn't actually belong to `disk_name`; `400` if it isn't actually a partition at all (a whole disk name given where a partition was expected); `409` if it's part of the OS disk's own layout, still has a role assigned (`DELETE /storage-roles/{name}` first, same no-silent-data-loss convention role removal already has elsewhere), or is currently mounted.

A disk is used in exactly one of two mutually-exclusive modes: role assigned directly to the whole disk (the original model), or partitioned with roles assigned to the individual partitions instead — `storage role.c`/`diskformat.c` needed no code changes of their own for this, since both already operate purely on whatever `GET /disks` reports, partition or whole disk alike.
...
It does **both halves** of the job: the table entry, then the filesystem inside it. Growing only the entry would leave the extra space invisible to everything using the filesystem, which reads as the resize having silently done nothing. ext4, btrfs and unformatted partitions are all grown (issue #163). The table entry is grown with `sfdisk --no-tell-kernel` and the kernel is then told about **just this one partition** via `BLKPG_RESIZE_PARTITION` — no whole-disk re-read, so the grow works on a disk that carries other mounted partitions (issue #94), the same mechanism the append path uses. The filesystem half depends on the type: `btrfs` grows *mounted* (the opposite of `resize2fs`), so an unmounted one is mounted at a private scratch point and unmounted again, while an **already-mounted btrfs is grown online, in place, on its live mountpoint** — the one path that extends the daemon's own data directory (`/var/lib/cix`, which can never be unmounted). `ext4` grows unmounted: `resize2fs` requires a clean filesystem, so `e2fsck -f -p` runs first; preen mode makes only the automatic, unambiguous repairs and refuses anything needing a human, and if it refuses, the partition table is left unchanged and you are told to check by hand. A mounted **non-btrfs** partition is refused (`409`) — unmount it first.

Only free space **immediately after** the partition can be used. Free space elsewhere on the disk cannot extend it, so a `409` here is about adjacency, not about the disk being full. Re-issuing the same request after a filesystem-grow failure is safe and idempotent: the table is already the right size, so it finishes the filesystem grow rather than refusing as a shrink.
...
/storage-roles`) and be currently mounted. `disk` is required in a migrate
...
**A placement is identified by filesystem UUID, not by the disk's name (#255).** A kernel device name like `sda` is assigned in probe order, so it is a *location* rather than an identity — it moves when the driver set, the controller set or the set of attached disks changes. That is not theoretical: adding SCSI low-level drivers to the kernel config renamed this platform's scratch disk from `sda` to `sdb`, every record naming it went stale at once, and the daemon would not boot. The UUID is read from the filesystem when a placement is set (and when a disk is formatted), stored alongside the name, and resolved back to whatever name the disk currently answers to at every boot. A placement made before this existed adopts its UUID automatically on the next boot. The `disk` field still reports a name, because a name is what you use everywhere else — it is simply no longer what the record *is*.

**A placement whose disk is missing no longer stops the daemon (#256).** It used to refuse to start, and on a host with no shell that removed the only means of correcting the record that was wrong — the API is the way in, and the API was what had refused to run. Recovery needed console access and the other A/B slot. Now the daemon starts on the default OS-disk location for that kind, reports `"degraded": true` with a `degraded_reason` on the placement's own `GET`, and logs the reason to the log store once it is open (it is resolved before the log store exists, and stderr is never mirrored into it — #132). **Nothing on the configured disk is deleted**; it is simply not consulted for that boot, so images and packages living there will not be visible until the placement is corrected and the host rebooted. A daemon that will not start cannot be told anything, which is the whole argument.
...
Both storage layouts migrate. An overlay container's `upper`/`work` are copied as ordinary trees; a snapshot container (ADR-0207) keeps its whole state in one btrfs subvolume, which is reproduced as a **real subvolume** on the target rather than flattened into a plain directory ([ADR-0234](../adr/0234-snapshot-containers-migrate-across-disks.md)). Snapshot containers were refused outright before that — copying a subvolume into an ordinary directory looks like it worked and leaves something that can never be snapshotted or quota-limited again, so refusing loudly beat corrupting quietly.
...
- `PUT /volumes/{name}/quota` gives a volume a real, kernel-enforced size limit — an ext4 project quota on its own directory, the same mechanism container overlays have had since [ADR-0062](../adr/0062-ext4-project-disk-quotas.md), so a write past it fails with `EDQUOT` at the filesystem level rather than being noticed afterwards. Without one, a volume can grow until its disk is full, and it becomes an unbounded thing to copy when scheduled backups run. On a **btrfs**-backed volume the mechanism is a **qgroup** instead — the same one a container's own rootfs quota uses ([ADR-0103](../adr/0103-btrfs-quota-backend.md)). A qgroup attaches to a subvolume, so volumes are created as subvolumes (#364); one created before that is an ordinary directory and setting a limit **converts it in place first**, preserving its contents. That conversion is refused `409` while a container mounting the volume is **running**, because a bind mount resolves once at container start ([ADR-0183](../adr/0183-persistent-volumes.md)) and pins the directory's inode — a live container would keep writing into the tree being replaced. Stop it, set the limit, start it again; clearing a limit (`0`) never needs a subvolume and is never blocked. Until #364 this was refused outright on btrfs, which was right at the time but meant no volume on the default substrate could be bounded at all; a volume that is still a plain directory and could not be converted is still refused, for that original reason. It is re-applied automatically after a migrate, since the project-quota tag lives on the directory and a volume that moved would otherwise arrive untagged and unlimited.

- `GET /volumes/{name}/usage` answers the one question the volume record could not: how much it actually holds. `{"name", "bytes", "source", "quota_bytes", "measured_at"}`, plus `limit_bytes` and `accounting` when `source` is `qgroup` — the limit is echoed so a caller has both halves of "how full" in one request. **Not a field on `GET /volumes`**, deliberately: one of the two ways to measure is a full tree walk, O(files), and the list is what the dashboard renders — a field there would walk every volume on every render. **Two sources, and `source` says which answered** (#369): a btrfs subvolume with quotas enabled is read straight from the kernel's own qgroup counter (`"qgroup"`, two tree lookups); anything else — a plain directory, a non-btrfs volume, or a subvolume on a filesystem where quotas were never enabled — is walked (`"walk"`). **They do not mean the same thing.** A walk sums apparent file content, the same accounting (and the same walker) as a container's own `disk.usage` on *its* walk path — and since #475 a container prefers its qgroup for the same reason a volume does. A qgroup counts *exclusive allocated extents*, so sparse, compressed and reflinked data read smaller, and it settles at the next transaction commit (~30 s) where a walk is instant — but it is the better number wherever it exists, because it is exactly what a `MAX_EXCL` limit is enforced against, and a walk can report a volume comfortably under a quota the kernel is about to refuse the next write on. On that path `limit_bytes` is the limit the *kernel* is enforcing, deliberately a separate number from `quota_bytes` (this daemon's record of what was asked for) so drift between them is visible; `accounting` is `"ok"`, `"stale"` (mid-rescan or flagged inconsistent, so `bytes` may read low) or `"simple"` (squota, different attribution rule). `df` inside the container is not an alternative and never will be: a volume is a bind of a directory, `statfs(2)` reports the superblock, so `df` shows the whole backing filesystem — true of a container's rootfs too, even one carrying a real qgroup limit, because btrfs does not reflect qgroups in `statfs`. `cixctl volume usage NAME`; the dashboard fills a **Size** column per row after the table is on screen, cached, rather than having the list carry it.
...
**A network with no `router` gets an explicit suppressing option, not silence.** dnsmasq's documented default when the router option is absent is to send *its own* address — which would point clients at the container serving DHCP, a machine that does not route. So a network with no gateway renders `dhcp-option=tag:<network>,option:router`. Note the PUT is a *partial update* over what is already stored, in the same way as every other config PUT here: omitting `router` keeps the value already set rather than clearing it, so pass `"0.0.0.0"` to remove one.
...
**The container half exists now (#260).** A container opts in with `"ksm": true` on `POST /v1/containers`, or `cixctl run --ksm`. The daemon applies it with `prctl(PR_SET_MEMORY_MERGE)` in the container's own process just before exec, which covers memory the payload has not allocated yet and is inherited by anything it forks — the payload is an arbitrary program with no call sites this daemon could add a `madvise()` to. Off unless asked for, because merging is not free: a container whose memory is genuinely unique pays the scanner's comparison cost for no return. A failure to apply is reported and does **not** stop the container, since merging is an optimisation and refusing to start would trade a working container for it.

The two halves are independent and both are needed: opting a container in while the scanner is off does nothing, and running the scanner with nothing opted in does nothing. KSM merges nothing that has not volunteered, and no container currently volunteers — the per-container opt-in (`prctl(PR_SET_MEMORY_MERGE)`) is not built yet. Enabling the scanner right now costs CPU and saves nothing. That is exactly why it defaults to **off**, the opposite of zswap, which costs nothing until the box is already swapping.
...
{"stalls": [
 ...
 "store": {"path": "/config/state/control_plane_stalls.jsonl", "exists": true,
           "dev": 65028, "inode": 22, "size_bytes": 197904, "first_nul_offset": -1}}
```
...
The service check is deliberately not another inference from shared memory. The watchdog is a separate process that cannot be wedged by the one it watches, so it simply **asks**: a plain-HTTP `GET /v1/health` every few seconds with a short timeout, and two consecutive unanswered probes are recorded. Nothing about the daemon's own bookkeeping can make that answer wrong, which is the point — every other signal here is the daemon's opinion of itself.
...
**`store` says which file you are actually reading (issue #229).** The watchdog logs its own path, dev, inode and size to `/dev/kmsg` after every append; the reader reported none of that, and that asymmetry cost a real investigation. Measured on the reference host: the writer logged `stored to /config/state/control_plane_stalls.jsonl (dev 65028 ino 22 size 196970)` three minutes before a `GET /system/stalls` whose newest record was **four days old** — and whose records contained none of the `slow-pass`, `service-stall` or `service-recovered` events the writer had been emitting for days. A file holding none of those stopped being appended to before they existed, so the two sides were not looking at the same bytes. With only one side instrumented, "a different file" and "the same file the reader cannot show" were indistinguishable from the API. They are deliberately the same three facts in the same order, so the two can be compared by eye.

**A NUL byte used to hide every record after it, and `first_nul_offset` is why that is now visible.** The reader split the file with `strtok()`, which treats the buffer as a C string — so the first zero byte ended the scan permanently and silently, and the report froze at one timestamp while the file went on growing. That is what was actually wrong: measured on the reference host, `store` reported the **same `dev` and `inode` the writer was appending to**, with a rising `size_bytes`, while the newest record returned stayed four days old. Not a different file — the same one, read through a C-string function.

The scan is bounded by length now and a NUL ends a line the same way a newline does, so nothing after the damage is lost. A zero run is the expected damage rather than an exotic one: this file is appended to by a watchdog whose entire purpose is to be running when the machine is about to be reset by hand, and a reset mid-append leaves the size updated with the block still zeroes. The one file that must survive a hard reset is the one most likely to carry a zero tail from it. Surviving that is not the same as reporting it, so `first_nul_offset` says where it happened; `-1` means the file is clean.
...
`dns_servers` is optional, and omitting it is not the same as sending `[]` — see below. 0–3 IPv4 addresses (matches glibc's own `resolv.conf` `MAXNS`, the same `RESOLV_MAX_NAMESERVERS` constant `PUT /system/resolv` already enforces for the *host's* own resolver). When given, staged as a real `/etc/resolv.conf` (`nameserver a.b.c.d` per line) directly into the container's own overlay before its process ever `execve()`s — the exact same pre-clone3 staging mechanism `files` above already uses, just a dedicated field so an operator doesn't have to hand-construct resolv.conf syntax themselves. `400` if the request also supplies a `files` entry whose `path` is `/etc/resolv.conf` — an unresolvable ambiguity, surfaced loudly rather than one silently overwriting the other. Echoed back on every `GET /containers`/`GET /containers/{name}` response (id-only, like `files`), and survives `restart: "always"`'s own replay mechanism the same way every other creation-time field does.

**Omitting it is a request for the default, and an explicit `[]` is the only way to ask for none** ([ADR-0295](../adr/0295-a-container-in-the-directory-can-read-it.md), [#451](https://git.home.arpa/itdlabs/cix/issues/451)). A container that names no `dns_servers` is given the registered DNS server containers (`dns server register --container=NAME`, see below) that share a network with it, in registration order, up to 3 — so a container attached to `services` on a box running `dns-1` and `dns-2` there gets both, with no operator action.

Three things get no default, each for its own reason:

- **An explicit `dns_servers: []`** — an operator saying "no resolver", which stays sayable.
- **An operator-supplied `files` entry for `/etc/resolv.conf`** — the same question, already answered by hand. (Combining that with an *explicit* `dns_servers` is still the `400` above; supplying only the file is not.)
- **A container that is itself a DNS server.** A resolver's upstream is `PUT /v1/dns/forwarders`, not its own `/etc/resolv.conf`; staging one would be a second, conflicting answer to where that resolver sends queries.

**The default is read from each server's persisted definition, never from the running registry**, and that is the load-bearing part of the design rather than an implementation detail. Autostart runs in `depends_on` order, so among containers that declare no dependencies the order is arbitrary — a registry-backed lookup would find `dns-1` on the boots where it happened to start first and nothing on the others, which makes the original symptom intermittent instead of fixing it. Reading the definition gives the same answer on every boot and on every replay, including while the DNS server is stopped.

Two consequences worth knowing. A registered server whose definition gives it no explicit `ip` on the shared network **contributes nothing** — an auto-allocated resolver address is not stable across boots, so a `resolv.conf` pinned to a previous allocation would be confidently wrong rather than honestly absent; that case is logged naming the server, and the fix is one explicit `ip` in its definition. And because the default is recomputed on every replay while the stored request body keeps no `dns_servers`, **registering a DNS server later reaches containers created before it existed**, on their next start.

Earlier behaviour, for anyone reading an older host: ADR-0143 made this field deliberately explicit, on `PUT /system/resolv`'s no-auto-discovery precedent (ADR-0076). That was coherent for an arbitrary container and incoherent for one the platform publishes in its own DNS — `jump` carried `dns_register: true`, so its name resolved for everyone else while it had no `/etc/resolv.conf` at all. ADR-0295 reverses that one decision; the field, its `400`, and its staging mechanism are unchanged.

Before this field existed, a container had no DNS resolution capability from Cix at all — not even the host's own outbound resolver config was ever propagated into a container's namespace. A container that needs DNS and shares no network with a registered DNS server still has none unless its own image bakes one in, or `dns_servers` names one.
...
**A build in flight reports when it started and how long it has been going (#423).** `run_started_at` is Unix epoch seconds for the whole job, fetching included; `build_started_at` is the same for compilation, which is `null` for a run still fetching — and a run that never gets past fetching is among the most interesting, so the two are deliberately separate. `run_seconds` is that span computed by the daemon.

Both forms exist on purpose. The absolute timestamp is what renders a real start time, which a relative figure cannot; the server-computed elapsed is what a counter should tick from, because differencing an absolute daemon timestamp against the *client's* clock turns any skew between them into a wrong elapsed time from the first frame — the same reason `last_output_seconds_ago` is relative. All three are `null` unless a run is open, so a finished entry never reports the start time of a job that is over.

Before this, a build reported that its output was 60 seconds old but not that it was in its fortieth minute. Both clocks already existed on the entry — ADR-0272's own run clock and the stall detector's build clock — and neither was ever reported.

**A build or a fetch can be stopped, and only by a person asking (`POST /v1/pkg/cancel`, issues #213 and #239).** A build that will never finish — one whose own `configure` is waiting on stdin, which is exactly how `perl` hung — otherwise holds its chain slot until the daemon restarts, because nothing else can stop it. `GET /v1/pkg` has reported `last_output_seconds_ago` since issue #58; this is the verb that acts on it. **A stuck fetch counts, and used to not.** Cancel accepted a fetching entry, set the flag and stopped there, on the reasoning that a fetch is a subprocess rather than a container so there was nothing to kill — leaving the flag to whichever completion path ran next. If the fetch never completes, none ever runs: a fetch retrying an unreachable upstream held its chain slot indefinitely, every later install was refused `409`, and cancel returned `200` having changed nothing. The fetch child is now killed, which drives the ordinary completion path that honours the flag. **A build whose container has already vanished is the same shape from the other side (#339):** cancel used to set the flag and hand back a build-container name that no longer resolved in the registry — so neither the "stop the running container" nor the "already-exited, drive completion" path ran, and cancel returned `200` while the slot stayed held, recoverable only by a reboot. It now drives the completion directly in that case too; `pkg_build_container_chain_index()` derives the slot from the container's own name suffix, so the release does not depend on the registry still knowing the container, and `cancel_requested` makes the completion record it as `cancelled` and free the slot.
...
**The build sandbox is the `cix-builder` image** ([ADR-0198](../adr/0198-build-sandbox-is-a-real-image.md)), resolved through the ordinary image mechanism rather than living at a flat path beside it (this used to compare itself to a hostbuild's `--build-image=`, which [ADR-0304](../adr/0304-a-hostbuild-composes-its-build-environment-like-every-other-build.md) retired). It used to be an unversioned directory grown silently by every successful install, so what it contained was a function of one box's entire install history — expressible nowhere, comparable to nothing, and the reason a recipe could stop building with no visible change anywhere. Now `GET /v1/images/cix-builder` shows its version history like any other image, and a version can be rolled back to.
...
`PUT` is a **partial update** — any field left out of the body keeps its existing value; an explicit `"auth_token": ""` is the one way to clear an already-set token, and the token itself is never echoed back by either `GET` or `PUT`, only a derived `auth_token_set` boolean. `repo_kind` is one of `gitea`/`github`/`gitlab` — each forge has a genuinely different archive-download URL shape and auth convention, handled as three separate, explicit branches rather than one generic abstraction (only the `gitea` branch has been verified against a real forge from this project's own dev environment; `github`/`gitlab` follow each forge's own documented API but are unverified against a live account of either kind). `repo_url` needs only `<scheme>://<host>/<owner>/<repo>` — a real forge *browse* URL pasted verbatim (e.g. gitea's own `.../owner/repo/src/branch/<ref>/<path>`, exactly what the address bar shows while looking at a repo) works too (ADR-0133): only the first two path segments are ever read as owner/repo, anything after is ignored, so there's no need to trim it down by hand first. `POST /v1/pkg/sync` starts an async fetch of the configured repo's recipe tree and merges it into this host's own catalog — **merge/additive, never a mirror**: an already-known `(name, version)` is counted as `skipped`, never overwritten (recipe versions stay immutable, ADR-0107), so a sync can never discard a recipe a host already has. The one exception ADR-0107 allows — applying a `pkg_artifact_sha256` approval to an already-published version — is counted as `approved`, kept distinct from `added` so a sync that only propagates approvals does not report them as new recipes (#404). `202`, poll `GET /v1/pkg/sync` (`state`: `never`/`running`/`success`/`failed`, plus `added`/`approved`/`skipped` counts and an `error` string) for the outcome — `400` if no repo is configured yet, `409` if a sync is already running. `sync_interval_seconds` in the repo config (0 = disabled, the default) optionally re-arms an automatic periodic sync on top of the always-available manual `POST`.

**Installs are also backed by a local build-artifact cache and, optionally, a plain-HTTP precompiled-artifact server (ADR-0122)** — deliberately two separate, independently-configured things from the git-forge recipe repo above: recipes are small, versioned text that belongs in git; a compiled package artifact does not, and reusing the same git-forge fetch mechanism for binaries would mean checking them into that same git tree to make them fetchable. Every successful real build is cached locally (`<name>-<version>.tar.gz`, keyed exactly like the recipe itself — the *local* cache carries no architecture because a host has only one, and stamping it would claim a distinction that does not exist there); installing the identical package into a second image, or reinstalling after deletion, hits the cache instead of re-fetching and recompiling — `GET`/`DELETE /v1/pkg/cache` show/clear what's cached, `GET`/`PUT /v1/pkg/cache-config` show/set its size cap (always a real, enforced cap — least-recently-used entries are evicted first when a new one needs room). A recipe can additionally opt in to network-fetched precompiled artifacts by declaring a `pkg_artifact_sha256=` field alongside its usual `pkg_source=`/`pkg_sha256=` — when a plain-HTTP artifact server is configured (`GET`/`PUT /v1/pkg/artifact-config`, a bare `base_url` + optional bearer token, never a git API), an install first tries `<base_url>/<name>-<version>-<arch>.tar.gz` and verifies it against the recipe's own declared checksum before ever trusting it; only on a miss (no server configured, 404, checksum mismatch) does it fall back to the recipe's real source and a real compile, exactly as if the artifact tier didn't exist. A recipe that never sets `pkg_artifact_sha256=` never touches this tier at all.

**A built artifact can leave the host that built it, and can publish itself (ADR-0201, issue #129)** — the artifact tier above describes *pulling*; this is the other direction, and it exists because an artifact's default fate used to be to be forgotten. Two halves. First, **export**: a hostbuild's harvested output (`kernel`, `cix`, `isotools`) lands in a plain host directory that is deliberately never container-visible (ADR-0056), so `GET /containers/{name}/files` cannot reach it — it had no retrieval route at all, meaning the self-hosted kernel, roughly a 50-minute build at the end of a four-stage toolchain chain, existed as bytes in exactly one directory on one machine, excluded from backup and wiped by factory reset. `POST /v1/pkg/{name}/artifact/export` tars it (202, poll the GET, then pull `?offset=&length=` chunks), producing `<name>-<version>.tar.gz`. The *published* name additionally carries this host's architecture (`<name>-<version>-<arch>.tar.gz`, issue #183): the cache has made architecture part of an artifact's identity since its own ADR-0008 and deliberately never infers one, so a host that does not assert it publishes unlabelled artifacts that have to be stamped by hand afterwards — which really happened, repeatedly. Architecture is added at the single point that builds the request URL for **both** fetch and push, so the two directions cannot drift apart. This shares one state machine with the image export (issue #126) rather than duplicating it: one export runs at a time, of either kind, matched on both kind and name so an image and a package sharing a name can never be confused. Second, **publish on request**: `POST /v1/pkg/{name}/artifact/publish` sends an artifact that already exists to the configured artifact server — the recovery path for a push that failed, was disabled, or predated the server's configuration, none of which should cost a rebuild. A hostbuild's tarball is built from the installed tree first (same export machinery), and its completion queues the push. A package name is not unique -- it names one installed entry per image -- and given only a name this means the *hostbuild* entry, that being the artifact this host produced and the only kind whose tarball can be rebuilt from an installed tree; it used to mean whichever entry came first internally, so `kernel`, installed both in `base` and as a hostbuild, was refused with a 409 while its hostbuild artifact sat on disk, and no host-built kernel was ever published (#520). This endpoint ran in production before it appeared in this contract; it entered during the ADR-0218 cutover, whose generated dispatch refuses to route an operation the spec does not declare — the exact drift that ADR makes impossible from now on. **A successful push queues a second one: the artifact's detached minisign signature** (ADR-0279), uploaded beside it as `<name>-<version>-<arch>.tar.gz.minisig`, with a trusted comment of `cix pkg <name>@<version> sha256=<hex>` — binding the signature to that exact revision and those exact bytes, and covered by minisign's global signature so none of the three can be edited afterwards. That is what lets a host which did not build the package install it without rebuilding, rather than depending on a `pkg_artifact_sha256` line that only ever reached the publishing host (#403). **On the install side, a signature is an approval in its own right.** The artifact tier is tried when *either* the recipe carries a `pkg_artifact_sha256` *or* this host holds a trusted key, and where both exist both must pass. A recipe with a checksum behaves exactly as ADR-0122 made it — 624 artifacts are published unsigned and stay installable — while a recipe with none now has a way in at all, which is the cold-box case (#306, #403): a freshly installed host can install `glibc` from the cache instead of building it, without any approval line ever having reached git. The signature's trusted comment is compared in full against `cix pkg <name>@<version> sha256=<hex>` built from the bytes just downloaded; a comment that does not say exactly which package, revision and bytes it approves is not an approval of this artifact. A host with an empty trust store verifies nothing and falls back to building from source, which is correct rather than degraded. Artifact first, then signature — the reverse of the ISO rule, because an ISO may not be published unsigned and a package may. A host holding no release key publishes the artifact and logs that it went out unsigned; the publish is not failed for it. If the signature push fails, the artifact stays published and the log names it, because an artifact whose signature never arrived looks complete and is silently rebuilt on every other host.
...
**A container's own full runtime definition can be declared as one git-syncable DEPLOYMENT, not retyped by hand on every recreation (ADR-0151; renamed from "container recipe" in #371, because it describes what to run and where, not how to build anything)** — `POST /v1/deployments` with `{"name": "<name>", "content": "<a complete POST /containers body, as raw JSON text>"}`. Unlike an image recipe's own small declarative format, a deployment's content already IS the exact JSON `POST /v1/containers` accepts — the recipe's own `"name"` field must equal the name it's published under (a mismatch is rejected, 400), and `POST /v1/deployments/{name}/apply` renders it and creates the container via the identical code path a direct `POST /containers` uses: 201 with the new container. A recipe's `files[].content` may embed `{{SECRET:KEY}}` tokens anywhere inside a string value — `apply`'s own request body's `secrets` object (`{"KEY": "real value"}`) substitutes each one, correctly JSON-escaped, before the container is ever created, so a real credential (an LDAP bind password, say) never has to be committed to git; an unmatched token is left completely untouched, never silently swallowed. `GET /v1/deployments/{name}` always shows the raw, unsubstituted text.

**Applying a deployment whose image is not built yet waits for the image, rather than refusing (ADR-0270).** Installing a package into an image already builds it from its recipe when no artifact exists — the composition edge from image down to package forks by itself. The edge from deployment down to image did not, so an operator naming an unbuilt image got a `400 image rootfs does not exist` and had to go realize the image by hand and come back. It now returns **`202 Accepted`**:

```json
{"name": "dns-1", "state": "awaiting-image", "awaiting_image": "dnsimg"}
```

The apply declares that image's own image recipe into its manifest if the manifest is still empty, queues the image on the same rebuild queue a rolling package publish uses, persists the deployment as waiting, and returns immediately — it never holds the connection open across a package build. The container is created automatically the moment the image is realized, and the wait survives a daemon restart. Track it at `GET /v1/pipeline`, where the deployment reports stage `acquire`, status `blocked`, and `blocked_on: {"kind": "image", "name": "dnsimg"}`; if the image builds but the create then fails for an unrelated reason (a name taken since, a network deleted), it reports `failed` at the same stage with that error rather than sitting silently.

A waiting deployment is also visible as an ordinary container: since [ADR-0181](../adr/0181-persist-all-containers-restart-decoupled-from-existence.md) every definition is persisted, so `GET /v1/containers/{name}` answers from it and reports `"status": "stopped"` — a defined container that is not running. On its own that reads as "somebody stopped it", which for something that has never started once is true and tells nobody anything, so the same object carries **`awaiting_image`** naming what it is waiting for. Empty on every ordinary container. (The status stays `stopped` rather than becoming a new enum value, because every client already switches on that field; the pipeline view is the authoritative place to see *where* it is stuck.)

Three things are deliberately *not* covered by this:

- **A direct `POST /v1/containers` naming an unbuilt image still fails fast with 400.** That path is also the boot-time autostart replay and the crash-restart replay; making a primitive wait would turn a fast, honest failure into a silent hang in callers that never asked for one.
- **An image with no manifest entries and no image recipe is still a 400.** There is nothing to converge toward, so there is no honest way to wait — the error names both absences.
- **An image existing is not the same question as an image having content.** Every image is born with a valid current version — the hash of its own empty manifest — and a real, empty rootfs directory. The readiness test compares against that version rather than merely checking one exists, because conflating the two is how an empty image once passed as ready for every later build ([issue #109](https://git.home.arpa/itdlabs/cix/issues/109)).
...
**That live restart REBUILDS the container's rootfs from the new version** ([ADR-0277](../adr/0277-a-container-rootfs-is-derived-from-its-image.md)). Until that ADR it did not, and the omission made this whole feature inert: the rootfs was seeded once on first start and reused forever, so the pin moved, the container restarted, and it came back on the old tree with the API reporting the new version (#401). A container's rootfs is derived from its image — anything that must outlive an image version belongs on a **volume**, because everything else written inside the container is discarded when the version moves. The rootfs records which version it was seeded from in `<base>/rootfs.version` and is kept only while that matches the pin; a rootfs predating that marker is adopted rather than rebuilt, and corrects itself the next time its version moves.
...
`GET /v1/pkg/{name}` also shows `"depends"` — what the recipe declared this entry needs in order to *run*, as captured once when the job fetched (ADR-0302), not re-read from whatever recipe revision is highest now. Empty string when the recipe declared nothing. For a hostbuild entry the declaration is carried and never resolved: a hostbuild has no image to merge a dependency closure into, so it describes the artifact rather than anything that was installed ([ADR-0303](../adr/0303-a-hostbuild-carries-pkg-depends-it-does-not-resolve-it.md), issue #465).
...
An optional `"version"` field pins the hostbuild to a specific published recipe version exactly like `/pkg/install`'s own (omitted resolves to the highest available). There is no `build_image` field: the build container is composed from the recipe's own `pkg_build_depends=`, exactly as it is for an ordinary install ([ADR-0304](../adr/0304-a-hostbuild-composes-its-build-environment-like-every-other-build.md), issue #482) — so a hostbuild's environment is what its recipe declares rather than whatever a named image happens to contain. A hostbuild recipe *may* also declare `pkg_depends`, and it is recorded on the entry and returned in `PkgEntry.depends`, but it is never resolved or installed here — resolution means "install the closure into an image" and a hostbuild has no image to merge into (ADR-0303, issue #465). `202`, polled via `GET /pkg/hostbuild/{name}` (a thin wrapper over the same `GET /pkg/{name}` lookup, scoped to a reserved internal image name) exactly like an ordinary install. Once `state: "installed"`, the artifact lives on the host at a fixed, well-known path per recipe (`kernel.recipe` → a `bzImage`; `cix.recipe` → `cixd`/`cixctl`/`web/`/`cix-install`/`mkinstalleriso` plus a server-side-assembled `cixd-root.squashfs`; `isotools.recipe` → a self-contained `grub-mkrescue`/`sbsign`/`sbverify`/`xorriso`/`mformat`/`mcopy` toolchain) — never merged into any container image's rootfs. `PkgEntry`'s own `files[]` stays empty for a hostbuild entry always, by design (nothing to `pkg_delete()` for a plain host artifact) — `artifact_path`'s own directory listing is the real answer to what a hostbuild produced. A hostbuild already in `state: "installed"` is a bare 409 on a repeat call unless `"upgrade": true` is given and the recipe's own `pkg_version=` has actually moved on (ADR-0094, mirrors `/pkg/install`'s own `upgrade` field exactly). `cixctl pkg hostbuild <name> [--version=<v>] [--wait] [--deploy] [--upgrade]` is the CLI surface; `--deploy` reads the finished artifact and calls the existing, unmodified `/system/update` for you.
...
Every field is optional, and an empty body is now the *recommended* way to build a shippable ISO: the installer is passed no arguments at all and asks for the disk and the network on the console, listing the machine's own disks and NICs. Supplying fields builds an unattended-install ISO for one specific machine instead. (Unset fields used to become the literal `CHANGEME`, with the documented workflow being to edit the kernel command line at the GRUB menu — typed blind against placeholders, with no feedback until the installer refused to start.) `202`, polled via `GET /system/iso` (`state`: `none`/`building`/`ready`/`failed`, `iso_path` once ready). Reuses whatever the most recent `cix`/`kernel`/`isotools` hostbuild rounds already harvested — it does not trigger any of them itself, and fails fast (`400`) naming exactly which one is missing rather than a background failure the caller has to poll for to discover. Requires a real Secure Boot signing key pair at `<data-dir>/keys/cix-signing.{key,crt,cer}`, installed via `PUT /system/signing-keys` (ADR-0212 — see below; it was an out-of-band-only precondition until then, which no real Cix host had any way to satisfy) — deliberately never generated, fetched, or copied there by `cixd` itself (see ADR-0064: a release-signing private key must never propagate onto every deployed box, only whichever specific instance is actually cutting installer media). `cixctl iso build [--disk=... --ip=... --prefix=... --gateway=... --interface=...] [--wait]` / `cixctl iso status` is the CLI surface. When a release key is installed (`PUT /system/release-key`, ADR-0220) the finished ISO is also minisign-signed and `signature_path` names the `.minisig` beside it; without one the build still succeeds and that field stays `null`.

### Which version of a seeded package ships, and where its bytes come from

Three packages go on the media as artifacts — `glibc`, `zlib` and `dnsmasq` — and until [#495](https://git.home.arpa/itdlabs/cix/issues/495) the daemon took *whichever entry the package table held first*, which is whichever image happened to sort first. On a host running glibc 2.44-16 in five images, the media was offered `cix-builder`'s 2.44-14. For zlib the arbitrary pick was worse than arbitrary: `cix-builder`'s 1.3.2-10 has neither a cached artifact nor an approved checksum, while `jumpbox`'s 1.3.2-14 has both.

The rule now: the **newest installed version across every image** whose bytes this host can actually produce — one already in the local artifact cache, or one whose recipe carries an approved `pkg_artifact_sha256`, which is fetched from the configured artifact cache and checked against that approval before it reaches the media. A version with neither is not a candidate, because seeding it means shipping media that names an artifact nothing can supply. `pkg_version_compare()` orders them — the same comparator the drift report and `recipe_latest_version()` use, not a second answer to "which is newer".

A fetched artifact is written straight into the seed directory, not into the shared local cache: the seed is rebuilt on every ISO build, and a build child writing the cache would run its LRU eviction alongside the install pipeline doing the same in the same directory.

**This also changes what a `400` from `POST /system/iso` means.** What the daemon can settle from its own tables — a seed package installed nowhere, or no installed version of one with either a cached artifact or an approved checksum — is still answered synchronously, because retrying will not change it. The staging work itself (copying the 12.8 MB recipe tree, copying or fetching the three artifacts) now runs in the forked build child and a failure there fails the build, reported by `GET /system/iso`. Staging ran in cixd's own event loop until #495, with every other request waiting on it; putting a network fetch there would have made a round trip part of the control plane's latency.

One of those inputs is checked more strictly than the rest (#481). The control-plane root — `cixd-root.squashfs`, the payload this ISO installs — must carry squashfs's own on-disk magic, not merely be a readable path. Every other input here is a binary or a directory, whose presence is all the check can usefully establish; the root is the one that another process is periodically rewriting. Testing only `access(R_OK)` meant the *media* path validated less than `POST /system/update`, which refuses exactly those bytes — so an assembly interrupted part-way could be baked into an installer image, signed, published, and first noticed as a machine that does not boot. The `400` now says which of the two it is: an assembly writing the file right now (poll `GET /system/assembly`), or one that was interrupted while writing it (`POST /system/assembly` to build it again).
...
Published as `cix-installer-{version}-{release}-{arch}.iso` — the cache's own canonical naming, with the architecture in the name because a checksum cannot tell an aarch64 image from an x86_64 one. The leading `v` is stripped from the version so the cache's own name/version/release split agrees with every package published from the same tag. **`{version}` is the `cix` *artifact's* version, not the running daemon's (#434).** The ISO is assembled from `ARTIFACTS_DIR/cix` (its `mkinstalleriso` and `web`/`cixd` bytes) and `ARTIFACTS_DIR/kernel` — hostbuild output that is routinely *newer* than the running daemon on a box that has built a release it has not yet deployed. Stamping the daemon's own `CIX_BUILD_VERSION` there labelled the ISO — its signed `built_version` and its published name both — with a version it did not contain, publishable straight over a name already in the cache. Both now come from `pkg_hostbuild_artifact_info("cix")`, the version those artifacts were harvested at, and the published name follows the signed `built_version`.
...
`boot` also carries **`platform_devices`** (#305) -- `esp`, `root_a`, `root_b`, `config`, `containers`, each the block device that partition resolved to **this boot**, read from the GPT partition labels `cix-install.c` writes rather than assumed from a device name. An empty string means no partition carrying that label was found on any disk, which is the normal answer for a dev/test daemon that is not an installed system and the thing to look at on one that is.

A sixth field, **`root_disk`** (#427), names the whole disk those five were resolved *on* -- `vdb`, say, or `""` when the running root is not on a block device. A GPT label is not unique: attach a second Cix disk, or a clone of this one, and two partitions carry `cix-config`, with nothing in the label to say which belongs to the running platform. So the lookup searches the disk the kernel mounted `/` from first (all five platform partitions are on one disk by construction, and the kernel has already decided which disk that is), and falls back to scanning every disk only when there is no root disk to scope to -- which is the normal case for `cix-recover`, whose `/` is the ISO it booted from. Read `root_disk` alongside the five devices rather than instead of them: five device paths read off the wrong disk look exactly like five off the right one, and this is the field that tells them apart.

It is reported here because it was otherwise visible only on stderr -- the serial console on an installed box -- and the failure it guards against is a disk coming back under a different name, which is exactly the situation in which nobody has a serial console to hand. A fully-installed host used to panic in a reboot loop when its disk enumerated as `sda` instead of `vda`; the labels never changed, and now nothing downstream assumes the name. The log store is not an option for these: they are resolved before anything is mounted, and the store lives on the very partition being resolved. `cixctl boot` prints one line per partition.

**`GET /system/assembly` is where control-plane assembly reports itself ([ADR-0230](../adr/0230-the-five-lifecycle-domains.md), issue #182).** These fields used to sit on `/system/boot`, whose subject is *what booted* — reporting there on *what is being built* meant an operator watching a deploy had to know that assembly is a side effect of a hostbuild named `cix` and is observed somewhere else. ADR-0230's rule is that status belongs to the thing that is happening, never to a neighbour that happens to know about it; this is its first application, and the fields moved rather than being duplicated.

`started_generation`/`completed_generation`/`running` (ADR-0105) are a real freshness signal for `pkg hostbuild cix --deploy`'s own server-side follow-on assembly (ADR-0057) — `cixd-root.squashfs` existing at the hostbuild's `artifact_path` is not the same as it being *this* round's own fresh build, since that file is a leftover from whichever assembly last succeeded. `completed_generation` only ever advances on a real, confirmed success; `--deploy` captures it as a baseline before triggering anything and waits for it to advance past that baseline (`running` distinguishes "still working" from "gave up, that attempt failed") rather than trusting file-exists.

The four `image_*` fields (#481) are what is *actually at that path*, `stat()`ed on every request rather than remembered — and they exist because the three counters above are not. All three live in the daemon's memory, and deploying an assembled root ends in a reboot, so the normal state of a host that has just booted the thing it assembled is `{"running": false, "started_generation": 0, "completed_generation": 0}` — byte-identical to a host that has never assembled anything in its life. Both readings are honest about what *this daemon process* has done; neither says anything about the file, which is the thing the operator's next command stages. That gap is how a possibly-half-written root came within one expired auth token of being staged and booted.

So the file answers for itself. `image_present` and `image_bytes` are its `stat()`. `image_mtime` is the freshness fact that outlives the reboot: an operator who knows when they started a build can tell this root from an older one without needing the daemon to remember. `image_complete` asks the same question `POST /system/update` asks before staging — squashfs's own on-disk magic — so a client learns here whether that call can succeed, instead of discovering it from a `409`. None of the four is persisted, for the reason [`GET /system/iso`](#building-a-fresh-installer-iso-server-side)'s `built_version` is not: a stored claim about a file is a second source of truth able to disagree with the file.

Since #481 an assembly writes to `cixd-root.squashfs.partial` and `rename(2)`s onto the final name only once `mksquashfs` has exited successfully, so `image_present` implies `image_complete` on a current build, and a failed or interrupted assembly leaves the *previous* root in place — untouched, whole, and the one the machine is currently running from. `image_complete` stays reported anyway, because a host upgraded from an older build can still be carrying a fragment an interrupted assembly left at that path.
...
**`POST /system/assembly` asks for one (issue #308).** The GET above reported on something no operator could request. Assembly happened only as a side effect of a hostbuild named `cix` completing, so re-assembling a root — after building an image one of its staging arguments reads from, say — meant inventing a new `cix` version whose code had not changed, because `POST /pkg/hostbuild` answers an already-installed version with `409 {"error":"package is already installed"}`. That cost is not theoretical: staging kernel modules into the root ([ADR-0266](../adr/0266-the-module-tools-are-an-image-the-module-tree-is-the-kernels.md), issue #347) needs one assembly to run under a daemon that knows how to stage them, and reaching it consumed a version number that existed for no other reason.

It runs the same assembly the hostbuild path runs, from the same `cix` artifact directory, with the same arguments — nothing about how a root is assembled changes, it simply becomes reachable. `202` and returns immediately; poll the GET and wait for `completed_generation` to pass the value you captured beforehand, exactly as `--deploy` already does. `409` if an assembly is already running (refused rather than queued: two `mkbootroot` processes writing the same output squashfs produce a root that is neither). `404` if this host has never completed a hostbuild of `cix`, since ADR-0057 assembles with that round's own freshly built `mkbootroot` and there is none.
...
**This endpoint's own boot manager support has a real history worth knowing before trusting it.** It was written and tested against real systemd-boot, which does read `LoaderEntryOneShot`. Two days later, ADR-0215 replaced systemd-boot with this project's own freestanding `cix-boot.efi` (`image/src/cix-boot.c`) — a from-scratch UEFI boot manager that imitates systemd-boot's on-disk *entry* format but, until issue #469, imitated none of its EFI-variable interface. The daemon side kept writing a real, correctly-formed NVRAM variable; nothing on the machine ever read it back. Confirmed dead end to end on 192.168.15.95, 2026-09-14: armed `cix-b.conf`, rebooted (a genuinely fresh boot, not a stale response — `kmsg` showed the reset), and the machine came back on the slot it was already running. #469 fixed `cix-boot.c` to read and consume the variable, matching what this section always documented. `PUT /system/esp {"default": ...}` had the same problem for the same root cause -- `cix-boot.c` had never parsed `loader.conf` at all -- and #467 fixed that separately: a minimal freestanding glob matcher and a small `loader.conf` reader, proven live the same way boot-next was -- a pattern naming only the non-running slot genuinely steered a real reboot there. `timeout` stays a deliberate no-op: it bounds an interactive menu's countdown, and this bootloader has no menu at all, so there is nothing for it to bound.
...
{"path": "/var/lib/cix/rebuildable/artifacts/cix/cix-boot.efi"}
```

or `{"url": "...", "sha256": "..."}` for the daemon to fetch it instead — the same shape as `update`'s own `image_url`/`image_sha256`.

This is the one file every future boot depends on, for **both** A/B slots at once — unlike the root squashfs and kernel, there is no per-slot fallback if a bad binary lands here. So it is never touched by `update`, and the write itself is staged (written to a temp file on the ESP, fsync'd) then swapped in with two renames rather than truncated in place; the outgoing binary is kept as `\EFI\BOOT\BOOTX64.EFI.bak` (not restorable through this API — that needs the ESP mounted outside the running host, the same recovery path #469 itself needed once).
