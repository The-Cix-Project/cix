...
A real, immediate `SIGKILL` — no grace period, unlike `DELETE /containers/{name}` (which has real container-lifecycle semantics: the container's own restart policy, network/cgroup teardown, etc.). Refuses `pid 1` outright (`400`): killing init crashes the host, and since ADR-0246 `pid 1` is `cix-init`, the supervisor, rather than `cixd` itself.

**This daemon's own pid is killable when it is supervised** (ADR-0246), and refused (`400`) when it is not. That reversal is the point of the split rather than a loosened check: the blanket refusal was right while `cixd` *was* `pid 1`, because killing it was indistinguishable from destroying the host and there was no recovery to return to. Under `cix-init` the worker is deliberately disposable — it is restarted within seconds, and the containers it manages keep running the whole time, being re-adopted from their cgroups rather than recreated. "Restart the control plane" becomes an ordinary operator action instead of an outage. Unsupervised it stays refused, and that is not a leftover: a `cixd` running as `pid 1` that kills itself is a kernel panic. Note the successful self-kill has no observable response — the process is gone before it can reply, so the connection drops; poll `GET /v1/health` to watch the new worker come up. Every other pid is allowed, **including one that happens to belong to a running container** — killing a container's own init pid this way is exactly equivalent to that container crashing on its own; the existing `SIGCHLD`-driven exit handling already covers it correctly, restart policy and all. `404` for a pid that isn't currently running, `400` for a non-numeric path segment. `cixctl process ls`/`process kill PID` is the CLI surface.
