...
**The build sandbox is the `thinc-builder` image** ([ADR-0198](../adr/0198-build-sandbox-is-a-real-image.md)), resolved exactly the way a hostbuild's own `--build-image=` already is rather than living at a flat path beside the image mechanism. It used to be an unversioned directory grown silently by every successful install, so what it contained was a function of one box's entire install history — expressible nowhere, comparable to nothing, and the reason a recipe could stop building with no visible change anywhere. Now `GET /v1/images/thinc-builder` shows its version history like any other image, and a version can be rolled back to.

That has a consequence worth knowing: this image is grown by **every** ordinary install, not only by an explicit `pkg install --image=thinc-builder`, so a hostbuild's own inputs move when unrelated things are installed. That was always true; it is now visible. Its *manifest*, though, still lists only what was installed into it directly — the accreted union is real content without a declared list, which is the remaining half of issue #40.
