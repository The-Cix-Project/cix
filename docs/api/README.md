...
```json
{
  ...
  "disk": {"upper_bytes": 4096, "upper_measured_at": 1789470000, "read_bytes": 0, "write_bytes": 16384, "read_ios": 0, "write_ios": 4, "pressure": {"some": {"avg10": 0, "avg60": 0, "avg300": 0, "total_usec": 0}, "full": {"avg10": 0, "avg60": 0, "avg300": 0, "total_usec": 0}}},
  "networks": [{"name": "internal", "rx_bytes": 1024, "tx_bytes": 2048, "rx_packets": 12, "tx_packets": 9}]
}
```

- `cpu`/`disk.read_*`/`disk.write_*`/`networks[].*_bytes`/`networks[].*_packets` are cumulative counters (since the container started); `memory.*`/`disk.upper_bytes` are gauges (current value, not a delta). `disk.upper_bytes` is the size of the container's own **writable tree**, content only — **not** the diff from its image, despite what this document said until it was measured: containers built from the same image agree to the byte (#475), because on the btrfs substrate that tree is a subvolume *seeded* from the image (ADR-0207) rather than an overlay upperdir over it — **measured in the background, not while answering the request** (#474). Measuring it is an `nftw()` walk, O(files) and unbounded, and it used to run inline on every stats request, on a pid-1 daemon whose one event loop is the only way into the host. A request serves the last figure and starts a new measurement when that one has aged out, so `disk.upper_bytes` is **`null` until the first measurement lands** (normally the request after the first) and `disk.upper_measured_at` says when it was taken. Reporting `0` for "not measured yet" was the alternative and it is a lie a caller cannot detect.
