...
| GET | `/images/{name}/versions/{version}/manifest` | What one specific image **version** declares, snapshotted when it was produced (#398) | The image-level manifest is a *live* document. A container records the version it runs from and keeps running it, so "what is in this container" must be answered from the version — reading the live manifest describes what the image holds *now*, a different question with the same shape. A version produced before snapshots existed returns 404 rather than falling back, deliberately: the fallback would be a wrong answer with nothing marking it. And this is what the version **declares** — not proof the bytes reached any container (#395). |
