...
This recipe set builds directory-by-directory on purpose, so any recipe installing *some* of an upstream project's headers can carry the same defect silently. The check is the cheap half of what a compiler does: for each installed header, does every **unconditional** angle-bracket include resolve to a file in this image's include tree? Angle-bracket only (a quoted include follows different rules), unconditional only (an include inside `#if`/`#ifdef` is often meant not to resolve here, and flagging those would make the check noise — a file's own include guard is not treated as such a conditional, or nothing in a guarded header would be checked at all), and existence rather than compilability (whether the resolved header itself parses is the compiler's question).
