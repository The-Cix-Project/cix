...
```json
{
  ...
  "disk": {"upper_bytes": 4096, "upper_measured_at": 1789470000, "upper_source": "qgroup", "read_bytes": 0, "write_bytes": 16384, "read_ios": 0, "write_ios": 4, "pressure": {"some": {"avg10": 0, "avg60": 0, "avg300": 0, "total_usec": 0}, "full": {"avg10": 0, "avg60": 0, "avg300": 0, "total_usec": 0}}},
  "networks": [{"name": "internal", "rx_bytes": 1024, "tx_bytes": 2048, "rx_packets": 12, "tx_packets": 9}]
}
```

- `cpu`/`disk.read_*`/`disk.write_*`/`networks[].*_bytes`/`networks[].*_packets` are cumulative counters (since the container started); `memory.*`/`disk.upper_bytes` are gauges (current value, not a delta). **`disk.upper_bytes` has two possible sources and `disk.upper_source` says which answered — read it before using the number** (#475). This document used to describe the field as the diff from the container's image, and containers built from the same image disproved it: two on 192.168.15.95 both read exactly `29360580`, which a real per-container diff cannot do. On the btrfs substrate a container's writable tree is a subvolume *seeded* from its image (ADR-0207) rather than an overlay upperdir over it, so a walk of that tree sums the whole rootfs.

    - **`upper_source: "qgroup"`** — the writable subvolume's **exclusive allocated extents**: what this container has written that its image snapshot does not hold, which is the number the field always promised. Same accounting and same preference order as [`GET /volumes/{name}/usage`](#volumes), so two endpoints answering "how big is this tree" agree on what counts. One ioctl, so it is read live on every request and forks nothing — but it settles at btrfs transaction commit, so it can be ~30 s behind, and `upper_measured_at` is when it was *read*.
    - **`upper_source: "walk"`** — the summed content of the whole writable tree, which on the seeded-subvolume layout includes the image content. It is the only figure available where no qgroup answers (storage not on btrfs, quotas disabled, or a writable tree that is not a subvolume), and it is now *labelled* rather than passed off as a diff. On this path the figure is **measured in the background, not while answering the request** (#474): the walk is `nftw()`, O(files) and unbounded, and it used to run inline on every stats request, on a pid-1 daemon whose one event loop is the only way into the host. A request serves the last figure and starts a new measurement when that one has aged out, so `upper_bytes` is **`null` until the first measurement lands** (normally the request after the first). Reporting `0` for "not measured yet" was the alternative and it is a lie a caller cannot detect.
