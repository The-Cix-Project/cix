...
kanxeoctl --host=<install-ip> daemon-config set --port=8080
kanxeoctl --host=<install-ip> daemon-config set --enable-https
kanxeoctl --host=<install-ip> daemon-config set --management-network=lan1
```
