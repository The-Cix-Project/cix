...
- **`--keep-on-failure` + the files API is your post-mortem** (ADR-0175): `thincctl pkg install ... --keep-on-failure` preserves the failed build container (named `__pkgbuild-N` — check `thincctl ps`). Any file in it is then one REST call away: `curl "http://<box>/v1/containers/__pkgbuild-N/files?path=/build/make.log"` — including crashed binaries themselves, pulled locally for a real gdb session (this exact flow root-caused a corrupted-jump-instruction GCC codegen bug this project hit). `thincctl pkg resume` can continue a preserved build without redoing fetch+extract. Delete the container (`thincctl rm __pkgbuild-N`) when done — **it holds the single build slot; a new install can't start while it exists**, and a *hung* (not crashed) build likewise holds the slot forever until you `rm` it.
...
- **Recipes that self-fetch from the private Gitea** (e.g. the kernel recipe pulling its own `.config` via the authenticated raw-content API): the committed recipe carries a `REPLACE_WITH_REAL_TOKEN` placeholder, never a real credential. The working sequence: commit the new version with the placeholder → live-substitute a real, freshly-created scoped API token into that file directly on Gitea (Contents API PUT) **before the version's first sync** → sync + trigger the build → once the fetch phase completes (state leaves `fetching`), immediately revert the file to the placeholder and delete the temporary Gitea token. Both the substitution and the revert are ordinary commits on the repo's HEAD; `git fetch && git rebase` locally afterward keeps clones in sync.
