...
- **Build capabilities work.** `capability "CAP_SYS_ADMIN"` is read by name and granted. An engine too old to report names is refused rather than read as zero, so a `cbs` predating that will fail the publish rather than silently build without the capability. **Nothing carries `pkg_build_caps=` over for you in a conversion, and forgetting it is not a build error.** The build runs; it just runs without a cgroup tree or namespace rights. So a converted recipe's container-creating tests fail with `cgroup_create: mkdir: No such file or directory` and `ns_clone3: Operation not permitted`, which reads as a platform fault (cix#515). Check `grep pkg_build_caps` on the shell form before publishing the `.cbs`.
...
**Do not add parallelism the shell form did not have** — but do not blame it either. `jobs $jobs` is the right translation of `make -j"$(nproc)"`; where the shell ran a bare `make`, adding it is a behaviour change and should be a deliberate one. (A command that parallelises internally, like `go build`, is unaffected: `-j` is a make flag and was never what made it concurrent.)

This paragraph originally asserted that adding `jobs $jobs` to `make selftest` was what broke `cix@v2.57.244-2`, whose suite failed on `VOLUME RESULT: FAIL (21)`. **That was wrong, and the measurement that disproved it is the useful part:** `v2.57.244-3` ran the same selftest serially and failed identically, in 764,335 ms against the parallel build's 761,849 ms. Two and a half seconds apart on a twelve-minute run means the flag changed nothing — the suite's own target does not parallelise. A plausible cause, written down before it was tested, sent the next revision at the wrong thing and cost a second twelve-minute build. `v2.57.244-4`, with no `jobs` anywhere, then failed with MORE volume failures than either, settling it. The cause was a **dropped declaration**: the shell recipe carried `pkg_build_caps="CAP_SYS_ADMIN"`, and `-2` to `-4` had no `capability` line. Without that capability, cixd mounts no cgroup2 into the build container (`src/container.c`, the `mount_cgroup2` derivation) and the container cannot clone namespaces. So `-4`'s log carried 258 `cgroup_create` errors, 33 `cgroup.controllers: No such file or directory` and `ns_clone3: Operation not permitted`, and 28 tests failed. `v2.57.244-6` differs from `-4` only by `capability "CAP_SYS_ADMIN"` and a comment; it shows 0 of each and 92/92 PASS (cix#515). A second fix went on record in cix#515 before this one was found: that CPDL build containers cannot create cgroups. It was wrong, because the two build paths receive an identical container spec.
