...
This paragraph originally asserted that adding `jobs $jobs` to `make selftest` was what broke `cix@v2.57.244-2`, whose suite failed on `VOLUME RESULT: FAIL (21)`. **That was wrong, and the measurement that disproved it is the useful part:** `v2.57.244-3` ran the same selftest serially and failed identically, in 764,335 ms against the parallel build's 761,849 ms. Two and a half seconds apart on a twelve-minute run means the flag changed nothing — the suite's own target does not parallelise — and the volume failure was real and pre-existing. A plausible cause, written down before it was tested, sent the next revision at the wrong thing and cost a second twelve-minute build. `v2.57.244-4`, with no `jobs` anywhere, then failed with MORE volume failures than either, settling it. The failure's real cause is not parallelism and not the recipe: a CPDL build container cannot create cgroups where a shell one can (cix#515), so the suite's container-creating tests fail under one build path and pass under the other.
