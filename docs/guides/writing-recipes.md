...
- **`pkg_build_image`** — the image this recipe is meant to be *hostbuilt* in, for recipes that are hostbuilds at all (`cix`, `kernel`, `isotools`). Ordinary package recipes leave it empty. Which image can build what used to be convention rather than contract — `cix-builder` builds the control plane, `kernel-builder` builds kernels — enforced by nothing, so a hostbuild naming the wrong one passed every check and then died minutes later inside a build container with a compiler error instead of "that is not what that image is for" ([#182](https://git.home.arpa/itdlabs/cix/issues/182), [ADR-0230](../adr/0230-the-five-lifecycle-domains.md)). Declaring it makes `--build-image` optional (the declared one is used) and turns a wrong one into an immediate `400` naming both images.
...
A recipe can also be built as a standalone, host-side artifact instead of merging into a container image's rootfs — used for building the Linux kernel and for [self-hosted rebuilds of Cix's own control plane](building-cix.md) (ADR-0056). The recipe format is identical, and every prerequisite the *build* needs must already be installed onto the named `--build-image=` beforehand, via ordinary `pkg install` calls against that image, exactly as described in [Build images](#build-images) above — a hostbuild composes its build container from that image alone, so neither `pkg_depends` nor `pkg_build_depends` has any say in what it builds inside.
