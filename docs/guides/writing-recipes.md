...
**Carry every declaration over, not just the commands.** A shell recipe's `pkg_build_caps=` becomes one `capability "…"` line per capability, placed after `metadata { }` and before the first phase. Forgetting it produces no error. The build runs without a cgroup tree or the right to clone namespaces, and any step that creates containers fails in a way that reads like a platform fault. Measured on `cix@v2.57.244` ([cix#515](https://git.home.arpa/itdlabs/cix/issues/515)):
...
Nothing else differs between the two. Both formats hand cixd the same container spec (`start_build_container_spec()` in `daemon/src/pkg.c`); the declared capabilities are the only thing the recipe contributes to it, and cixd mounts cgroup2 into the container only when `CAP_SYS_ADMIN` is among them (`src/container.c`, the `mount_cgroup2` derivation). Parallelism was ruled out on the way: a serial selftest failed identically to a parallel one, 764,335 ms against 761,849 ms.
