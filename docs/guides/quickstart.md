...
```sh
build/kanxeoctl health
```

`{"status": "ok"}` means it's up. Full detail: [`building-kanxeo.md`](building-kanxeo.md).
...
```sh
build/kanxeoctl pkg bootstrap
build/kanxeoctl pkg recipe add --name=bash --file=pkg/recipes/bash.recipe
build/kanxeoctl pkg install --name=bash
```

Poll until it's done:

```sh
build/kanxeoctl pkg ls
```
...
```sh
build/kanxeoctl network create --name=lan1 --subnet=172.31.0.0 --prefix=24
```
...
```sh
build/kanxeoctl run --name=hello --image=base --network=lan1 -- /usr/bin/bash -c "echo hello from inside Kanxeo"
build/kanxeoctl inspect hello
```
