...
```sh
thincctl run --name=web --image=myapp --pki-issue --pki-cert-dir=/etc/thinc-tls --pki-days=365 -- /usr/bin/some-binary
```
...
```sh
thincctl run --name=svc1 --image=myapp --ldap-provision --ldap-group=svcaccts -- /usr/bin/some-binary
```
