...
That one call configures both halves (#419, [ADR-0282](../adr/0282-glauths-listeners-are-configuration.md)): `--server-tls` is rendered into each registered glauth's own config as `[ldaps] enabled`, and `--client-tls` points the clients at it. **A listener change does not yet reach a running glauth** — its config watcher reloads records, not sockets, and a restart re-stages the recipe's own value over the rendered one; see #419. Until that is closed, a listener change still needs the container recreated from a recipe carrying the value you want, which is what step 4 below does. `effective_client_uri` then reads `ldaps://<ip>:636/` instead of `ldap://<ip>:3893/`, and every consumer picks it up from that one derivation: the `nslcd.conf` the daemon renders for an `ldap_client` container (which also gains a `tls_cacertfile` line, since nslcd runs `tls_reqcert demand` and refuses everything without a trust anchor), and the `{{LDAP:URI}}` recipe token. Containers are restaged on their next create.
