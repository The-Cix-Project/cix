...
```sh
cixctl ldap config set --server-tls --server-tls-port=636 --client-tls
```

That one call configures both halves (#419, [ADR-0282](../adr/0282-glauths-listeners-are-configuration.md)): `--server-tls` is rendered into each registered glauth's own config as `[ldaps] enabled`, and `--client-tls` points the clients at it. glauth binds its listeners at startup and its config watcher reloads only records, so a **running** server adopts a listener change when it restarts — add `--restart-servers`, which restarts each registered server one at a time with the previous one back first. Off by default on purpose: a plain config PUT should never disrupt the directory that authenticates the control plane. `effective_client_uri` then reads `ldaps://<ip>:636/` instead of `ldap://<ip>:3893/`, and every consumer picks it up from that one derivation: the `nslcd.conf` the daemon renders for an `ldap_client` container (which also gains a `tls_cacertfile` line, since nslcd runs `tls_reqcert demand` and refuses everything without a trust anchor), and the `{{LDAP:URI}}` recipe token. Containers are restaged on their next create.
