...
- **System > Host > Disks** -- multi-disk management (ADR-0071/ADR-0102/ADR-0104, ADR-0140, ADR-0141): every real host block device, live from sysfs, flagging which one is the fixed OS disk (never a role/format candidate). "Assign role…" (also reachable via the header `+ Create > Disk Role`) binds a disk to `container-storage`, `backup`, `state-storage`, `rebuildable-storage`, or `log-storage` -- non-destructive, reversible via "Remove role" (refused, 409, if the disk is the active state-storage or log-storage placement below). Once a role is assigned, a per-row filesystem select + **Format…** button destructively `mkfs`s (ext4 or btrfs) and mounts the disk -- guarded by a `confirm()` dialog, since the disk is already unambiguous from the row it's on (the same "you already specified which one" reasoning `kanxeoctl disks format NAME` itself uses, not a second typed-name field); likewise refused (409) against either active placement. A container's own storage disk is chosen at creation time (`run --disk=NAME`), not from this page. Two placement sections above the table -- **State storage placement** and **Log storage placement** -- each show which disk (if any) currently holds that concern, with a select (populated from disks already carrying the matching role) and **Migrate** button to move it: live, no downtime, poll status shown inline once a migration starts, and the two migrate independently of each other (moving state doesn't block moving logs at the same time, or vice versa).
