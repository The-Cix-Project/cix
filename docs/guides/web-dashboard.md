...
- **System > Host > Disks** -- multi-disk management (ADR-0071/ADR-0102/ADR-0104, ADR-0140): every real host block device, live from sysfs, flagging which one is the fixed OS disk (never a role/format candidate). "Assign role…" (also reachable via the header `+ Create > Disk Role`) binds a disk to `container-storage` or `backup` -- non-destructive, reversible via "Remove role." Once a role is assigned, a per-row filesystem select + **Format…** button destructively `mkfs`s (ext4 or btrfs) and mounts the disk -- guarded by a `confirm()` dialog, since the disk is already unambiguous from the row it's on (the same "you already specified which one" reasoning `kanxeoctl disks format NAME` itself uses, not a second typed-name field). A container's own storage disk is chosen at creation time (`run --disk=NAME`), not from this page.
