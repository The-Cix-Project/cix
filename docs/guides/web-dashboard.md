...
- **System > PKI > Root CA / Intermediate CA** — once bootstrapped, a "Download certificate (.crt)" button next to each one's status line, alongside inline instructions for trusting it on your own device (Windows/macOS/Linux/Firefox all differ — see [`security.md`](security.md#trusting-the-ca-on-your-own-device) for the full steps). Purely a client-side save-as over `cert_pem`, already part of `GET /pki/ca`/`GET /pki/intermediate`'s own response — no separate download endpoint.
