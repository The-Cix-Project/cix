...
- **System > Host > Volumes** -- persistent volumes (issue #88, ADR-0183): storage whose lifetime is independent of any container, so it survives the container being deleted and recreated. Filed beside Disks rather than under Monitoring -- it is storage management, and it shares Disks' own role mechanism for placement. The table shows each volume's disk, resolved host path and creation time, with a create form above it and a per-row Delete that is a `confirm()`-guarded danger action: deleting a volume destroys real data that deliberately outlived its containers, and is refused outright (409, naming the container) while any container definition still references it.
