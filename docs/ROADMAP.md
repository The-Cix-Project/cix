...
| 4 | CLI (pure REST API client) | 3 | Next |
...
Verified end to end by `test/test_daemon.c` (`test/daemon_child.c` is its exec target; takes a sleep-seconds and an exit-code argument) — drives the built daemon purely over real HTTP (hand-written requests, `daemon/src/json.c` reused to parse responses rather than ad hoc string matching):
...
## Phase 4 — CLI, pure REST API client (next, not yet designed)

Will wrap the Phase 3 API in a command-line tool with zero direct runtime access — every subcommand is an HTTP call. Not designed yet.

## Phase 5 — Web dashboard, pure REST API client (not yet designed)
