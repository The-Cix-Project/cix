...
int main(int argc, char **argv)
{
	...
	if (argc != 14) {
		fprintf(stderr,
		        ...
		        "  kernel command line. Pass \"\" for a shippable ISO: the installer\n"
		        "  then asks for the disk and the network on the console, listing this\n"
		        "  machine's own disks and NICs, and no argument needs editing at all.\n"
		        "  This used to carry --disk=/dev/CHANGEME and friends, with the\n"
		        "  documented workflow being to edit the kernel command line at the GRUB\n"
		        "  menu with 'e' before booting -- typed blind, against placeholders,\n"
		        "  with no feedback until the installer refused to start. Supply real\n"
		        "  values here only to build an unattended-install ISO for one specific\n"
		        "  machine.\n"
