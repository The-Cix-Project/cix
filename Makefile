...
# ADR-0246: the supervisor. Deliberately its own tiny binary with no
# daemon or runtime-library sources -- it must not be able to acquire
# the dependencies that make the worker able to block.
$(BUILD)/cix-init: init/src/cix_init.c | $(BUILD)
	$(CC) $(CFLAGS) init/src/cix_init.c -o $@
