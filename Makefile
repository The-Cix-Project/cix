...
SELFTESTS = \
	...
	$(BUILD)/test_docindex $(BUILD)/test_web_vt $(BUILD)/test_web_tree $(BUILD)/test_web_syntax $(BUILD)/test_toolchain_policy \
	$(BUILD)/test_clitree \
	...
	$(BUILD)/test_json $(BUILD)/test_jsondiff $(BUILD)/test_nsswitch \
...
DAEMON_SELFTESTS = \
	...
	$(BUILD)/test_management_address $(BUILD)/test_storage_placement \
...
#
# ADR-0296 (#478): pure logic over the two nsswitch.conf variants --
# no daemon, no filesystem -- so it runs in a build container and is
# in SELFTESTS. It gates the convergence decision, which no runtime
# test can reach: a successful package install needs a real build
# container, which is the one thing a build container cannot make.
#
$(BUILD)/test_nsswitch: test/test_nsswitch.c daemon/src/nsswitch.c | $(BUILD)
	$(CC) $(CFLAGS) -Idaemon/include $^ -o $@

$(BUILD)/test_json: test/test_json.c daemon/src/json.c | $(BUILD)
	$(CC) $(CFLAGS) -Idaemon/include $^ -o $@

$(BUILD)/test_jsondiff: test/test_jsondiff.c daemon/src/jsondiff.c daemon/src/json.c | $(BUILD)
	$(CC) $(CFLAGS) -Idaemon/include $^ -o $@
...
# ADR-0251's package-finalize policy, embedded into the daemon.
#
# The policy is authored as shell (daemon/policy/pkg-finalize.sh) so it
# is reviewable and directly runnable -- test_pkg_finalize runs that
# exact file -- and embedded here so cixd carries it and can never be
# asked to build a package without it. No build tool needed: the
# transform is escape backslashes, escape quotes, wrap each line.
$(BUILD)/generated/pkg_finalize.h: daemon/policy/pkg-finalize.sh
	@mkdir -p $(BUILD)/generated
	@printf '/* Generated from %s -- do not edit. See ADR-0251. */\n' $< > $@
	@printf 'static const char PKG_FINALIZE_SH[] =\n' >> $@
...
# ADR-0251: runs daemon/policy/pkg-finalize.sh -- the same file the
# daemon embeds -- against synthetic DESTDIR trees. No daemon, no
# container, no binutils: strip is stubbed so the classification is
# what is asserted.
$(BUILD)/test_pkg_finalize: test/test_pkg_finalize.c daemon/policy/pkg-finalize.sh | $(BUILD)
	$(CC) $(CFLAGS) test/test_pkg_finalize.c -o $@

# ADR-0253: a build output tree is created fresh, never inherited.
# Asserts the one operation mkbootroot, mkinstalleriso and the hostbuild
...
$(BUILD)/test_srcupstream: test/test_srcupstream.c daemon/src/srcupstream.c daemon/src/kernelpolicy.c daemon/src/persist.c daemon/src/json.c | $(BUILD)
	$(CC) $(DAEMON_CFLAGS) $^ -o $@

$(BUILD)/test_srcpolicy: test/test_srcpolicy.c daemon/src/srcpolicy.c daemon/src/srcdepth.c daemon/src/srcupstream.c daemon/src/kernelpolicy.c daemon/src/persist.c daemon/src/json.c | $(BUILD)
	$(CC) $(DAEMON_CFLAGS) $^ -o $@
...
$(BUILD)/test_web_tree: test/test_web_tree.c | $(BUILD)
	$(CC) $(CFLAGS) test/test_web_tree.c -o $@
...
$(BUILD)/test_timebounds: test/test_timebounds.c | $(BUILD)
	$(CC) $(CFLAGS) test/test_timebounds.c -o $@

$(BUILD)/test_procfuse: test/test_procfuse.c daemon/src/procfuse.c daemon/src/proctitle.c | $(BUILD)
	$(CC) $(CFLAGS) -Idaemon/include $^ -o $@

# ADR-0248: gcc as a linter, never as a producer. -fsyntax-only, so it
# emits no object and nothing it touches can reach an artifact -- TCC
# still compiles every byte this project ships.
$(BUILD)/test_lint: test/test_lint.c | $(BUILD)
	$(CC) $(CFLAGS) test/test_lint.c -o $@

$(BUILD)/test_listenbind: test/test_listenbind.c | $(BUILD)
	$(CC) $(CFLAGS) test/test_listenbind.c -o $@

$(BUILD)/test_blocking_waits: test/test_blocking_waits.c | $(BUILD)
	$(CC) $(CFLAGS) test/test_blocking_waits.c -o $@

$(BUILD)/test_bootorder: test/test_bootorder.c | $(BUILD)
	$(CC) $(CFLAGS) test/test_bootorder.c -o $@
...
$(BUILD)/mkbootroot: image/src/mkbootroot.c test/test_image_fixture.c daemon/src/persist.c daemon/src/osrelease.c $(BUILD)/version.h | $(BUILD)
...
#
# netconf.c: the net.conf format, which cix-install writes at install
# time and cixd both reads at boot and writes on PUT /v1/system/
# management-network. One definition, linked by both, rather than a
# writer in each program.
$(BUILD)/cix-install: image/src/cix-install.c image/src/dual_console.c daemon/src/treecopy.c daemon/src/partlabel.c daemon/src/netconf.c daemon/src/kmod.c daemon/src/json.c | $(BUILD)
	$(CC) $(CFLAGS) -Idaemon/include $^ -o $@
...
#
# cix-init: pid 1 in every container (ADR-0260). Freestanding -- no
# libc, no headers, its own _start -- and therefore built with neither
# $(CFLAGS)'s include path nor the CRT. -static is load-bearing:
# without it tcc adds a PT_INTERP (probe-tcc-conformance 27). This is
# the one binary in the tree linked -static, named as such in
# CLAUDE.md's toolchain rule; test_cix_init checks the result carries no
# PT_INTERP and no DT_NEEDED so the property cannot rot quietly.
#
